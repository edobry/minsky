...
import { acquireMarker, releaseMarker, startMarkerHeartbeat } from "./inflight-marker";
...
export async function runReview(
  ...
  // mt#4993: no-op until a marker is actually held, so the finally below can
  // call it unconditionally on every path — including the fail-open one, where
  // no marker exists and nothing should be stopped.
  let stopMarkerHeartbeat: () => void = () => {};
  ...
  if (deps.db !== undefined) {
    try {
      ...
      // mt#4993: hold the marker for as long as this review runs. Without the
      // heartbeat the marker expires on a static TTL while the review is still
      // going, and acquireMarker's expiry-takeover (mt#4267) then hands the row
      // to a second caller — 19 duplicate concurrent reviews over 30 days.
      // Started here rather than inside acquireMarker so the lease's lifetime is
      // the try/finally below, which is what actually bounds the work.
      stopMarkerHeartbeat = startMarkerHeartbeat(deps.db, {
        markerId: markerResult.id,
        deliveryId,
      });
  ...
  try {
    ...
    // mt#4993: stop the lease BEFORE releasing, not after. A beat landing in
    // the window between the DELETE and the stop would find no row, report
    // ownership lost, and log a heartbeat_ownership_lost warning for what is
    // actually an ordinary clean finish — turning the signal that measures this
    // defect into noise generated by its own fix.
    stopMarkerHeartbeat();
