 ...
 *
 * 1. **Separate process.** The worker runs inside the Railway-deployed
 *    `minsky-reviewer-webhook` service — a different process on a different
 *    host than any implementer Claude Code session. No shared filesystem,
 *    no shared memory, no inherited environment.
 * 2. **Separate identity.** Submissions post under `minsky-reviewer[bot]`
 *    (App ID 3470137) — a different GitHub App than `minsky-ai[bot]`
 *    (the implementer App). The App-identity split is enforced by
 *    `getAppIdentity` (`github-client.ts`) and rotates Octokit tokens per
 *    installation — see ADR-006.
 * 3. **Diff + spec + read-only access only.** Inputs to the model call
 *    (assembled below by `fetchPullRequestContext` + `resolveTaskSpec` +
 *    `buildReviewPrompt`) are exactly: the PR diff, the task spec, the
 *    Critic Constitution, and prior reviews. The model has read-only
 *    file access via the curated `tools.ts` allowlist (no write tools).
 *    No implementer session transcript, chat history, or intermediate
