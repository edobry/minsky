...
name: incident-memo
description: >-
  Synthesis-level postmortem across a multi-incident working session, producing
  a Notion page plus a companion memory entry. Triggers at 2+ retrospectives, 3+
  merged PRs, or an explicit ask for a day synthesis. Cross-incident layer;
  /retrospective is per-incident.
user-invocable: true
---
