...
- **Never decode preset codes or build preset URLs manually.** Use `npx shadcn@latest preset decode <code>`, `preset url <code>`, or `preset open <code>`.
...
6. **Fix imports in third-party components** — After adding components from community registries, check for hardcoded import paths. Use `npx shadcn@latest info` to get the correct aliases.
7. **Review added components** — Always read the added files and verify they are correct.
8. **Registry must be explicit** — When the user asks to add a block or component, do not guess the registry. If no registry is specified, ask which registry to use.

## Updating Components

When the user asks to update a component from upstream while keeping their local changes:

1. Run `npx shadcn@latest add <component> --dry-run` to see all files that would be affected.
2. For each file, run `npx shadcn@latest add <component> --diff <file>` to see what changed upstream vs local.
3. Decide per file based on the diff — apply upstream updates while preserving local modifications.
