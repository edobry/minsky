...
description: >-
  ...
  "ok") at a task-graph junction, check whether multiple unblocked sibling/child
  tasks exist; if so, surface the choice in user-facing output instead of
  walking to a self-derived default. Use after a task merges / transitions to
  DONE when ≥ 2 sibling or child tasks become newly walkable.
user-invocable: true
---
