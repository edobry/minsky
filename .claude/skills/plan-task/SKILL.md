...
- An explicit recommendation to use a named external tool/library/service not already
  present in the project's dependency manifest
...
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| License       | `gh api repos/<owner>/<repo>` → `.license.spdx_id`                                                                                                          | `Proprietary` / `NOASSERTION` / `Other` / `GPL-*` (any GPL variant) — block until explicit acknowledgment from user         |
...
| Install path  | Probe registry: `curl -s https://pypi.org/pypi/<name>/json \| jq .info.name` for Python; `npm view <name> version` for Node; `cargo search <name>` for Rust | 404 / no result — block unless spec provides the correct alternative install path (e.g., `git clone && pip install -e .`)   |
| Canonical URL | HTTP 200 on the spec's stated URL AND on any repo/install URL found inside the upstream's own README                                                        | Disagreement between spec URL and upstream README URL, OR 404 inside upstream README — block                                |
