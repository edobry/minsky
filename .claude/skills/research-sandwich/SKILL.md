...
description: >-
  ...
  (plan), fresh subagents each research one workstream in parallel (fan-out),
  and a Fable advisor assembles the results into one coherent deliverable
  ...
  synthesize." NOT for a single-fact lookup or a one-source question —
  sandwiching a one-liner is waste.
user-invocable: true
---
