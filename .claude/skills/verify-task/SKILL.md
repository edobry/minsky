...
name: verify-task
description: >-
  Set DONE on the bypass-merge fallback path: confirm the PR merged AND the
  merge commit carries the canonical bypass audit signature, then transition
  IN-REVIEW → DONE. The reviewer bot does the verification at review time; this
  only confirms the closeout signal. Use for "verify mt#X" / "close out mt#X".
user-invocable: true
---
