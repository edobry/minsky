...
name: learn
description: >-
  Route task-acquired reusable knowledge to the durable artifact that owns it —
  memory, skill, rule, or doc — then land it through the right channel. Invoke
  as `/learn <what you learned>`, or self-trigger on noticing knowledge that
  outlives the current task. Bounded to the current turn; never blocks work in
  progress.
user-invocable: true
---
