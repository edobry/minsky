...
function decide(input: Input, deps: { collaborator: Collaborator }) {
  // ...returns something assertable
}
```
...
obvious injection seam (functional-core/imperative-shell extraction, one wiring test per shell),
and the Bun-specific mechanics (engine-internal symbol-table mutation, no `restoreMocks`
equivalent, one shared module registry) that make an unrestored spy in this runtime a correctness
bug, not just a lint nit. `testing-standards.mdc §Testable Design` has the injection pattern in
more depth.
...
### Automated Pattern Enforcement (TEMPORARILY DISABLED)
