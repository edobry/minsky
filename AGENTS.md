...
- **Context-fill gauge** (mt#4291) — reports the session's own context fill. DISPLAY-ONLY (ask#8878), log-only. `MINSKY_SKIP_CONTEXT_FILL_GAUGE`. Detail: `context-fill-gauge.md`.
...
- **Claim provenance** (mt#4168) — a spec asserting a file-level COLLISION or a NEGATIVE OWNERSHIP claim at the spec-WRITE seam with no discharging call, MCP or shell. An audit RECORD is not a claim; discharge is per-paragraph. RECORD-ONLY. `MINSKY_SKIP_CLAIM_PROVENANCE`. Detail: `claim-provenance-scan.md`.
