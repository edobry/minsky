...
- **Turn-end-untaken-action** — Stop scan (mt#3179): final message names a next action without taking it. Phrase-keyed to MATCH; its two suppressions are not, and are still RECORDED. Speaks over ask-routing-deferral on overlap, carrying that guard's remedy. `MINSKY_ACK_UNTAKEN_ACTION`. Detail: `turn-end-untaken-action-scan.md`.
...
- **Operator deferral** — an ACTION deferred to the principal without a same-turn capability probe; sibling of ask-routing-deferral, which covers a DECISION. SIX surfaces, one act-path; evaluation stream over all six, so the miss RATE is measurable. Calibration-first (mt#2459). `MINSKY_SKIP_OPERATOR_DEFERRAL`. Detail: `operator-deferral-detector.md`.
...
- **Nonexistent-search-path** (mt#4215) — a `grep`/`rg`/`find` path argument that does not exist; empty output is not absence. Silent when unresolvable. Calibration-first. `MINSKY_SKIP_NONEXISTENT_SEARCH_PATH`. Detail: `nonexistent-search-path-detector.md`.
