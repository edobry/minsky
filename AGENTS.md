...
entrypoints): 17 guards share ONE Bun process per event (`GUARD_REGISTRY`); others are standalone
`settings.json` entries. **Load-bearing:** timeout is the HOST CAP, guards run SEQUENTIALLY —
size to the SUM of pre-migration timeouts. Source `.minsky/hooks/`; `.claude/hooks/*` is
GENERATED (`compile --target claude-hooks`; commit both). Override:
`MINSKY_HOOK_OVERRIDE=<guard>[,...]|all` (migrated guards only). Doc: `guard-dispatcher-framework.md`.

## Parallel-Work Guard

PreToolUse on `session_start`/`tasks_dispatch`/`tasks_create`: blocks session-binding on open-PR
file overlap (advisory for recently-merged); blocks duplicate-child titles vs an ACTIVE sibling
(terminal WARN); parent-less creates get a WARN-only similarity probe. Hook:
`parallel-work-guard.ts`. Overrides: `MINSKY_FORCE_PARALLEL=1` / `MINSKY_FORCE_DUPLICATE_OK=1`
(launch-time) + mid-session grant `--guard parallel-work-open-pr` / `duplicate-child-matcher`.
Fail: closed (open-PR) / open (duplicate checks). Doc: `parallel-work-guard.md`.
...
a `gh api PUT .../merge` bypass unless every required check concluded `success` on HEAD; only an
UNKNOWN (unreadable) status consults the D8 grant store, a confirmed failure never does. Hook:
`require-checks-on-bypass-merge.ts`. Overrides: `MINSKY_SKIP_REQUIRED_CHECKS=1` (launch-time) or
a D8 grant (`scripts/grant-guard-override.ts`). Fail: deny-on-failure. (Shared PR data-fetch
layer for the 4 `session_pr_merge` gates: `pr-data-fetch-layer.md`.)
...
`Deploy verification:` marker (shared form: label+colon or any-level heading, case-insensitive);
paired PostToolUse injects a `deployment_wait-for-latest` reminder. Hooks:
`deploy-surface-detector.ts` + `require-deploy-verification-before-merge.ts` +
`deploy-verification-after-merge.ts`. Escapes: `[no-deploy-impact]` title;
`MINSKY_SKIP_DEPLOY_VERIFY=1`. Fail: open (unresolvable repo/PR/non-surface); PostToolUse always
exits 0.
...
justification:` marker. Hook: `require-growth-justification-before-merge.ts`. Override:
...
- **Workspace-COPY** — regenerates protected Dockerfiles' COPY block from the root `workspaces`
  glob, auto-restaging; no override; blocks only on missing markers.
...
`operator-instruction-after-merge` (telling the operator to reinstall/rebuild/edit-config to
activate a merged change). Hook: `substrate-bypass-detector.ts`. Overrides:
...
`turn-end-retro-scan.ts` runs an advisory turn-end scan (via `dispatch-stop.ts`), dedup caps
phrases at one beat, same override. Doc: `turn-end-retro-scan.md`.
...
blocks when the spec was never surfaced (`tasks_spec_get` / `tasks_get includeSpec:true`). Hook:
...
- **Silent-stretch** — flags a tool-only stretch (no assistant text) crossing 10 min OR 15
  consecutive tool calls. `silent-stretch-detector.ts`; override `MINSKY_SKIP_SILENT_STRETCH=1`.
...
unexpired capability grant (`scripts/grant-subagent-merge.ts --task mt#<id> --ttl-minutes 30`).
...
matches a live one-shot ask-grant AND the referenced `authorization.approve` Ask re-verifies
server-side as operator-approved, emits `permissionDecision: allow` with an audit reason. Issue
(TTL 15m, one-shot): `bun scripts/grant-ask-action.ts --ask <id> --command-exact "<cmd>"`. Hooks:
...
grant-present-but-unverified. Sibling denies outrank this allow. Doc: `ask-permission-bridge.md`.
...
escalation guard (+ cockpit widget) warns every turn while critical. Hooks: `guard-health.ts` +
...
dispatch surfaces' `intent` param (default `"implementation"`, no-op). Default-ALLOW. Hooks:
