...
- **Turn-end-untaken-action** — Stop scan (mt#3179): final message names a next action without taking it. Phrase-keyed to MATCH; its two suppressions are not — a named principal-reserved category, and tool-call state showing a wait armed past the turn's end. Speaks over ask-routing-deferral on overlap, carrying that guard's remedy. Suppressed fires are still RECORDED. `MINSKY_ACK_UNTAKEN_ACTION`. Detail: `turn-end-untaken-action-scan.md`.
...
- **Operator deferral** — an ACTION deferred to the principal without a same-turn capability probe; sibling of ask-routing-deferral, which covers a DECISION. SIX surfaces, one of them act-path — keyed on tool-call state rather than prose. Evaluation stream over all six, fired or not, so the miss RATE is measurable. Calibration-first (mt#2459). `MINSKY_SKIP_OPERATOR_DEFERRAL`. Detail: `operator-deferral-detector.md`.
...
- **Claim provenance** (mt#4168) — a spec asserting a file-level COLLISION, or a NEGATIVE OWNERSHIP claim, written at the spec-WRITE seam with no discharging call (`pull_request_read` or its shell equivalent, path-filtered `git_log`, or a preceding `tasks_search`). An audit RECORD is not a claim (ADR-024 Rung 1); discharge is per-paragraph (mt#4190). RECORD-ONLY. `MINSKY_SKIP_CLAIM_PROVENANCE`. Detail: `claim-provenance-scan.md`.
