...
added by commit `77537c806` on 2026-08-05. `v1.3.1` is the newest release, so custom-scheme
dispatch ships only on unreleased `main`/`tip`. Re-check in one command:

```
curl -sL -o /dev/null -w '%{http_code}\n' \
  https://raw.githubusercontent.com/ghostty-org/ghostty/{v1.3.1,main}/macos/Sources/Helpers/UntrustedURL.swift
```
...
- **Claim provenance** (mt#4168) — a spec asserting a file-level COLLISION or a NEGATIVE OWNERSHIP claim at the spec-WRITE seam with no discharging call. RECORD-ONLY. `MINSKY_SKIP_CLAIM_PROVENANCE`. Detail: `claim-provenance-scan.md`.
