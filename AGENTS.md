...
- **Retrospective-trigger** — reminds `/retrospective`; Stop sibling `turn-end-retro-scan`. Full ADR-024 ladder as of mt#3652: Rung-1 regex fires directly; Rung-2 embedding nomination stays LOG-ONLY (`MINSKY_RUNG2_NOMINATION_ENFORCE` untouched); Rung-3 Haiku confirm judges nominations and a CONFIRMED nomination injects. Fails to Rung-1 on any confirm failure. ALSO writes an EVALUATION stream, `.minsky/retrospective-trigger-evaluations.jsonl` (every evaluated turn, fired or not — the mt#3583 pattern; separate file, since coverage-receipt reads calibration-record existence as fire-evidence), closing the gap where a Rung-1 miss wrote nothing. `MINSKY_ACK_RETROSPECTIVE_TRIGGER`; Rung-3 kill switch `MINSKY_DISABLE_RUNG3_CONFIRM`.
