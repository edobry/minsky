...
**Branch-protection management.** Layer 2's desired config is declarative in `scripts/set-branch-protection.ts`. Dry-run is the default: `bun scripts/set-branch-protection.ts` previews live-vs-desired diff. `--check` exits non-zero on drift (suitable for periodic audits). `--apply` writes the desired config via `gh api PATCH`.
