...
- **Turn-end-untaken-action** — Stop scan (mt#3179): final message names a next action without taking it. Phrase-keyed to MATCH; two suppressions are not — the message NAMES a principal-reserved category (mt#3768), or the turn's TOOL CALLS show a wait armed past its end (mt#4063). On overlap with ask-routing-deferral THIS guard speaks (mt#3620) and emits the deferral remedy (mt#3767). Suppressed fires are still RECORDED. `MINSKY_ACK_UNTAKEN_ACTION`. Detail: `turn-end-untaken-action-scan.md`.
...
- **Operator deferral** — an ACTION deferred to the principal without a same-turn capability probe. Sibling of ask-routing-deferral (which covers a DECISION). SIX surfaces, prose and tool-call-state both; the act-path one (mt#4081) has a blocking sibling in `block-bulk-process-kill`. Evaluation stream over all six, fired or not, so the miss RATE is measurable. Calibration-first (mt#2459). `MINSKY_SKIP_OPERATOR_DEFERRAL`. Detail: `operator-deferral-detector.md`.
...
- **Unrendered-result-field scan** (mt#3913) — `session_pr_create` on a branch adding a counter/flag to a `*Result` type that no output site renders. **A log call is not a render site.** Diff-only, no corpus; narrowed mt#4147 (a `*Result` declared under `**/hooks/**` or `**/detectors/**` is no longer considered). Calibration-first. `MINSKY_SKIP_UNRENDERED_RESULT_FIELD_SCAN`. Detail: `unrendered-result-field-scan.md`.
...
- **Flakiness-control detector** (mt#3658) — `tasks_create` whose spec claims a failure MODE with no isolation control recorded. Fires on the ATTRIBUTION ("flaky", "load-dependent") and equally on the DENIAL ("not load-dependent", "fails deterministically"). Silenced PER CLAIM by its family's evidence (mt#4166): attribution by counts, DENIAL only by a literal `Load control:` record; `UNVERIFIED` excuses only its own claim. Calibration-first. `MINSKY_SKIP_FLAKINESS_CONTROL`. Detail: `flakiness-control-detector.md`.
