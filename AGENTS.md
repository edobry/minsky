...
**How it works:**
1. On a `tasks_create` with `parent` set, enumerate the parent's children via
   `minsky tasks children <parent>` + a per-child `minsky tasks get <id> --json`
   (capped at `TASKS_CHILDREN_FETCH_CAP = 60`). **ALL statuses** are enumerated
...
warn-and-permit; an individual unreadable/malformed child is skipped (not fatal).
