    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-3 p-4">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs text-muted-foreground">
          Readable agent-session transcripts. Open one to read its conversation as a tab.
        </p>
      </div>

      {query.isLoading && <p className="text-sm text-muted-foreground">Loading sessions…</p>}
      {query.isError && (
      ...
      {!query.isLoading && !query.isError && sessions.length === 0 && (
        <p className="text-sm text-muted-foreground">No ingested session transcripts yet.</p>
      )}
