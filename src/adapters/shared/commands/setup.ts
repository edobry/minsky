...
const setupParams = composeParams(
  ...
  {
    // mt#5153: shared with `init` — one enum, one refusal, one prompt.
    client: CLIENT_PARAM,
    ...
    defineCommand({
      ...
      execute: async (params, _ctx) => {
        try {
          ...
          // Determine which client to register with (mt#5153): the same
          // resolution `init` uses. Until now this branch never consulted the
          // environment or the MCP caller — an agent's `minsky setup` under
          // CLAUDECODE=1 on a multi-client machine failed where it could have
          // resolved — and with NO client installed it defaulted to `cursor`,
          // the one silent default `setup` still carried.
          const resolvedClient = await resolveClientForCommand({
            explicit: params.client,
            callerActorId: params.callerActorId,
            flag: "--client",
            promptMessage: "Select an MCP client to register Minsky with:",
          });
          ...
          const result = await performSetupFn(
            { repoPath, client, harnessSource: resolvedClient.source, overwrite },
            undefined,
            {},
            {},
