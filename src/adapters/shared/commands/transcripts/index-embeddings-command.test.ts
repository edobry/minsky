...
import { registerTranscriptIndexEmbeddingsCommand } from "./index-embeddings-command";
import type { AppContainerInterface } from "@minsky/domain/composition/types";

const COMMAND_ID = "transcripts.index-embeddings";
...
describe("transcripts.index-embeddings command", () => {
  let registry: ReturnType<typeof createSharedCommandRegistry>;

  beforeEach(() => {
    registry = createSharedCommandRegistry();
    registerTranscriptIndexEmbeddingsCommand(undefined, registry);
  });
  ...
  describe("registration", () => {
    test(`registers under id ${COMMAND_ID}`, () => {
      ...
      expect(command.name).toBe("index-embeddings");
      expect(command.category).toBe(CommandCategory.TRANSCRIPTS);
    });

    test("description mentions both per-turn and summary indexing", () => {
      const command = getCommand();
      expect(command.description).toContain("per-turn");
      expect(command.description).toContain("summary");
    });

    test("declares all, session, and force parameters", () => {
      const command = getCommand();
      ...
      expect(params.force).toBeDefined();
    });

    test("all and force default to false; session has no default", () => {
      const command = getCommand();
      const params = command.parameters as Record<string, { defaultValue?: unknown } | undefined>;
      expect(params.all?.defaultValue).toBe(false);
      expect(params.force?.defaultValue).toBe(false);
