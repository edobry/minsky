  ...
  command.action(
    async (options: {
      ...
      try {
        ...
        // Use repo from normalized repo options, falling back to cwd
        ...
        const params = {
          ...
          ...outputOptions, // Include debug flag
        };
        
        // Call domain function - it doesn't return a value
        await initializeProjectFromParams(params);
        
        // Create a default success result
        const result = { success: true, message: 'Project initialized for Minsky.' };
        
        // Output result using shared utility
        outputResult(result, {
          json: options.json,
          formatter: () => {
