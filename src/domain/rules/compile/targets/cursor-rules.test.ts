...
import { GENERATED_BANNER, GENERATION_BANNER_PATTERNS } from "../banner-constants";
...
describe("cursor-rules target", () => {
  ...
  describe("serializeRuleToMdc()", () => {
    ...
    // matched by `GENERATION_BANNER_PATTERNS` — the shared pattern set used
    // by `.claude/hooks/check-generated-file-edit.ts`. Because both sides
    // import from the same `banner-constants` module, drift is structurally
    // impossible; this regression guard pins the runtime invariant that the
    // emitted banner matches at least one pattern (the hash-comment one).
    describe("generated-file banner (mt#1798)", () => {
      ...
      it("banner matches at least one hook detection pattern (drift guard)", () => {
        ...
        // We expect the hash-comment pattern specifically for cursor-rules.
        expect(matchedPattern?.name).toBe("hash comment: Generated by");
      });

      it("banner survives complex YAML frontmatter (block scalars, arrays)", () => {
        // Edge-case test: ensure banner-injection does not interfere with
        // YAML block-scalar formatting or array layout for the closing `---`.
        const rule = makeRule("complex-rule", "Body content", {
          name: "Complex Rule",
        ...
        expect(lines[0]).toBe("---");
        expect(lines[1]).toBe(GENERATED_BANNER);
        // Frontmatter still parses round-trip cleanly: there should be exactly
        // two `---` delimiter lines and the body content follows the closer.
