...
/**
 * Generated-file banner emitted as a YAML comment inside the frontmatter block
 * (mt#1798). Form chosen for Cursor compatibility: Cursor's `.mdc` parser is
 * strict about line-1 being the `---` frontmatter opener, so an HTML comment
 * before the frontmatter would risk the file being silently skipped. A YAML
 * `#` line-comment inside the frontmatter is invisible to Cursor's parser and
 * matched by the `check-generated-file-edit` hook's existing `# Generated by`
 * regex pattern. Kept in sync with the equivalent constant in
 * `src/domain/rules/compile/targets/cursor-rules.ts`.
 */
...
export function buildRuleMdc(rule: RuleDefinition): string {
  ...
  // Inject the generated-file banner as the first frontmatter line. The opener
  // is always exactly `---\n` per gray-matter; replace the first occurrence.
  return raw.replace(/^---\n/, `---\n${GENERATED_BANNER}\n`);
}
