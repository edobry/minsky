...
const sessionTempDir = path.join(
  Bun.env.SESSION_WORKSPACE ||
    process.env.SESSION_WORKSPACE ||
    "/Users/edobry/.local/state/minsky/git/local/minsky/sessions/task#077",
  "test-tmp"
);
...
describe("initializeProject", () => {
  ...
  test("should create directories and files for tasks.md backend and cursor rule format", async () => {
    ...
    expect(mockFS.mkdirSync).toHaveBeenCalledWith(path.join(repoPath, "process", "tasks"), {
      recursive: true,
    });

    expect(mockFS.mkdirSync).toHaveBeenCalledWith(path.join(repoPath, ".cursor", "rules"), {
      recursive: true,
    });
  ...
  test("should create directories and files for tasks.md backend and generic rule format", async () => {
    ...
    expect(mockFS.mkdirSync).toHaveBeenCalledWith(path.join(repoPath, ".ai", "rules"), {
      recursive: true,
    });
  ...
  test("should only configure MCP when mcpOnly is true", async () => {
    ...
    const tasksFileCalls = mockFS.writeFileSync.mock.calls.filter((args) =>
      String(args[0]).includes("tasks.md")
    );
    expect(tasksFileCalls.length).toBe(0);

    // Should create MCP config
    const mcpConfigCalls = mockFS.writeFileSync.mock.calls.filter((args) =>
  ...
  test("should overwrite existing files when overwrite is true", async () => {
    ...
    const mockFS = createMockFileSystem({
      [path.join("/test/repo", "process", "tasks.md")]: "Existing content",
      [path.join("/test/repo", ".cursor", "rules", "minsky-workflow.mdc")]: "Existing rule",
    });
    ...
    const tasksFileCall = mockFS.writeFileSync.mock.calls.find(
      (call) => String(call[0]) === path.join("/test/repo", "process", "tasks.md")
    );
