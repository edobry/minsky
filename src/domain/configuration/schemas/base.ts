...
export const baseSchemas = {
  ...
  postgresConnectionString: z.string().regex(
    /^postgresql:\/\/[^:]+:[^@]+@[^:]+:\d+\/[^?]+(\?.*)?$/,
...
export const enumSchemas = {
  ...
  aiProvider: z.enum(["openai", "anthropic", "google", "cohere", "mistral"], {
    errorMap: () => ({ message: "AI provider must be one of: openai, anthropic, google, cohere, mistral" })
  }),
  
  // Detection rule conditions
  detectionCondition: z.enum(["tasks_md_exists", "json_file_exists", "always"], {
    errorMap: () => ({ message: "Detection condition must be one of: tasks_md_exists, json_file_exists, always" })
  }),
} as const;
