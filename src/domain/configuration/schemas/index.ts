...
export const configurationValidation = {
  ...
  hasRequiredFields: {
    ...
    ai: (config: Configuration): boolean => {
      ...
      return !!(
        (providers.openai?.apiKey || providers.openai?.apiKeyFile) ||
        (providers.anthropic?.apiKey || providers.anthropic?.apiKeyFile) ||
        (providers.google?.apiKey || providers.google?.apiKeyFile) ||
        (providers.cohere?.apiKey || providers.cohere?.apiKeyFile) ||
        (providers.mistral?.apiKey || providers.mistral?.apiKeyFile)
      );
    },
  ...
  validateConsistency: (config: Configuration): { valid: boolean; errors: string[] } => {
    ...
    if (config.backend === "github-issues") {
      if (!config.backendConfig["github-issues"]?.owner || !config.backendConfig["github-issues"]?.repo) {
