      ...
      if (!fs.existsSync(filePath)) {
        ...
        if (result.backupsFound.length > 0) {
          result.suggestedActions.push({
            ...
            command: `minsky persistence migrate --from ${result.backupsFound[0]?.path ?? ""} --to ${expectedFormat}`,
            autoExecutable: true,
            priority: "high",
          });
        } else {
      ...
      if (actualFormat === expectedFormat) {
        ...
        if (actualFormat === "sqlite") {
          result.suggestedActions.push({
            ...
            command: `minsky persistence migrate --from ${filePath} --to ${expectedFormat}`,
            autoExecutable: true,
            priority: "high",
          });
        }
