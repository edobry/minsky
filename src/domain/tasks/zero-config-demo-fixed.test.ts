/**
 * Fixed Zero-Config Experience Demo Test
 * 
 * This test demonstrates the complete zero-config workflow enabled by the
 * configuration system, using the correct data format for each backend.
 */
...
describe("Zero-Config Experience (Fixed)", () => {
  test("tasks can be listed without --backend flag when repository is configured for json-file", async () => {
...
`;
      ...
      // Create tasks data in the json-file backend format
      // The json-file backend uses .minsky/tasks.json as default location
      const tasksState = {
        tasks: [
          {
            id: "#001",
      ...
      await fs.writeFile(join(minskyhDir, "tasks.json"), JSON.stringify(tasksState, null, 2));
  ...
  test("zero-config experience with markdown backend auto-detection", async () => {
