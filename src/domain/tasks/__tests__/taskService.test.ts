...
import type { TaskData, TaskReadOperationResult, TaskWriteOperationResult } from "../../../types/tasks/taskData.js";
...
function createMockBackend(): TaskBackend {
  return {
    ...
    getTasksData: mock(() => Promise.resolve({
      success: true,
      content: "- [ ] Task 1 [#001](#)\n- [+] Task 2 [#002](#)",
      filePath: "mock/tasks.md"
    } as TaskReadOperationResult)),
    
    getTaskSpecData: mock((specPath: string) => Promise.resolve({
      success: true,
    ...
    formatTasks: mock((tasks: TaskData[]) => {
      ...
      return tasks.map(t => `- [${t.status === "DONE" ? "x" : " "}] ${t.title} [${t.id}](#)`).join("\n");
    }),
    ...
    formatTaskSpec: mock((spec) => `# Task ${spec.id}: ${spec.title}\n\n## Context\n\n${spec.description}`),
    
    // Mock side effect methods
    saveTasksData: mock(() => Promise.resolve({
      success: true,
      filePath: "mock/tasks.md"
    } as TaskWriteOperationResult)),
    
    saveTaskSpecData: mock(() => Promise.resolve({
      success: true,
...
describe("TaskService", () => {
  ...
  describe("listTasks", () => {
    ...
    test("should return empty array if data retrieval fails", async () => {
      ...
      mockBackend.getTasksData = mock(() => Promise.resolve({
        success: false,
  ...
  describe("setTaskStatus", () => {
    ...
    test("should throw error for invalid status", async () => {
      await expect(taskService.setTaskStatus("#001", "INVALID")).rejects.toThrow(/Status must be one of/);
    });
  ...
  describe("createTask", () => {
    ...
    test("should throw error if spec file read fails", async () => {
      ...
      mockBackend.getTaskSpecData = mock(() => Promise.resolve({
        success: false,
        error: new Error("Test error"),
        filePath: "path/to/spec.md"
      }));
      
      await expect(taskService.createTask("path/to/spec.md")).rejects.toThrow(/Failed to read spec file/);
    });
  });
  
  describe("backend handling", () => {
    test("should throw error for non-existent backend", () => {
      expect(() => new TaskService({ backend: "nonexistent" })).toThrow(/Backend 'nonexistent' not found/);
    });
