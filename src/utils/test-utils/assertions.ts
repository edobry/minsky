 ...
 *
 * Note: Many of these wrap Bun's built-in matchers (toHaveLength, toBeInstanceOf,
 * not.toBeNull, toMatch, toBeCloseTo) for callers that use these functional forms.
 * New tests should prefer Bun's native `expect(x).toHaveLength(n)` style.
 */
...
export function expectToHaveLength(value: unknown, length: number): void {
  expect((value as { length: number }).length).toBe(length);
}
...
/** Mock function shape used for call-tracking assertions */
interface TrackableMock {
  mock?: { calls: unknown[][] };
}
 ...
 */
export function expectToHaveBeenCalled(mockFn: TrackableMock): void {
 ...
 */
export function getMockCallArg(mockFn: TrackableMock, callIndex = 0, argIndex = 0): unknown {
...
export function expectToHaveProperty(object: unknown, propertyPath: string, value?: unknown): void {
  ...
  for (const part of parts) {
    ...
    expect(part in currentObj).toBeTruthy();
