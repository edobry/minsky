...
export async function autoCommitTaskChanges(
  ...
  try {
    ...
    const statusResult = await execGitWithTimeout("check-status", "status --porcelain", {
    ...
    const filesToStage = ["process/tasks.md", "process/tasks/"];
    ...
    const stagedResult = await execGitWithTimeout("check-staged", "diff --cached --name-only", {
      workdir: workspacePath,
    });
    ...
    await execGitWithTimeout("commit", `commit -m "${message.replace(/"/g, '\\"')}"`, {
      workdir: workspacePath,
    });
