...
describe("SemanticErrorClassifier Integration Tests", () => {
  ...
  describe("Real filesystem scenarios", () => {
    it("should correctly classify file not found vs directory not found", async () => {
      ...
      const fileError = new Error(`ENOENT: no such file or directory, open '${join(testDir, "existing-dir", "missing-file.txt")}'`);
      ...
      const dirError = new Error(`ENOENT: no such file or directory, open '${join(testDir, "missing-dir", "file.txt")}'`);
    ...
    it("should provide context-aware solutions for real scenarios", async () => {
      const error = new Error(`ENOENT: no such file or directory, open '${join(testDir, "deep", "nested", "file.txt")}'`);
      (error as any).code = "ENOENT";
