 ...
 * An MCP-only agent has no parameter path to set this var directly:
 * `session_commit` has no env-passthrough parameter, and a raw `git commit`
 * is blocked by the block-git-gh-cli.ts guard. The SANCTIONED mid-session
...
export class PreCommitHook {
  ...
  private async runRulesCompileCheck(): Promise<HookResult> {
    ...
    // file — the dir legitimately holds hand-authored rules too, which must
    // not trigger the staleness check; reviewer R1)
    try {
      const fsp = await import("fs/promises");
      const rulesDir = `${this.projectRoot}/.claude/rules`;
      const entries = await fsp.readdir(rulesDir);
      for (const entry of entries) {
        if (!entry.endsWith(".md")) continue;
        try {
          const content = String(await fsp.readFile(`${rulesDir}/${entry}`, "utf-8"));
          if (
            content.includes("<!-- Generated by minsky rules compile. Do not edit directly. -->")
          ) {
            targetsToCheck.push("claude-rules");
            break;
          }
        } catch {
          // Unreadable entry — keep scanning others.
        }
      }
    } catch {
      // Directory doesn't exist — skip
