  ...
  try {
    ...
    if (config.ai?.providers) {
      ...
      for (const [name, providerConfig] of Object.entries(config.ai.providers)) {
        ...
        console.log(
          `    apiKey: ${(providerConfig as any)?.apiKey ? `${(providerConfig as any).apiKey.substring(0, 8)}...` : "none"}`
        );
      }
    }
    ...
    console.log(
      "Morph API key:",
      morphConfig?.apiKey ? `${morphConfig.apiKey.substring(0, 8)}...` : "missing"
    );
    ...
    const fastApplyProviders = Object.entries(config.ai?.providers || {})
      .filter(([name, providerConfig]) => (providerConfig as any)?.enabled && name === "morph")
      .map(([name]) => name);
