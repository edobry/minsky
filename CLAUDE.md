...
- **Nonexistent-search-path** (mt#4215) — a nonexistent `grep`/`rg`/`find` path argument; empty output is not absence. Calibration-first. `MINSKY_SKIP_NONEXISTENT_SEARCH_PATH`. Detail: `nonexistent-search-path-detector.md`.
