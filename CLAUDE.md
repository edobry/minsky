...
entrypoints): guards share ONE Bun process per event — 17 migrated (`GUARD_REGISTRY`;
mt#2650/2652/2687/2812/2824); all other guards below are standalone `settings.json` entries.
**Load-bearing:** a dispatcher's `timeout` is the HOST CAP and guards run
...
`MINSKY_FORCE_DUPLICATE_OK=1` (launch-time-env-only) + the mid-session reason-mandatory grant
channel (mt#2658): `--guard parallel-work-open-pr` (open-PR sweep, mt#1637) /
`--guard duplicate-child-matcher`; probe has none. Fail: closed
...
Not a permission gate — surfaces guard-layer failures (mt#2812): dispatcher errors/check-skips
...
every turn while any is critical. Hooks:
