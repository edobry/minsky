...
- **Retrospective-trigger** — reminds `/retrospective`; Stop sibling `turn-end-retro-scan`. Full ADR-024 ladder as of mt#3652: Rung-1 regex fires directly; Rung-2 embedding nomination stays LOG-ONLY (`MINSKY_RUNG2_NOMINATION_ENFORCE` untouched — mt#3408's 3/3-FP measurement stands); Rung-3 Haiku confirm judges nominations and a CONFIRMED nomination injects (pilot: 6/6 positives, 0/7 FPs incl. all three mt#3408 FP sentences; first live corpus fire hand-classified TP — the mt#3639 prettier admission). Fail-to-Rung-1 on any confirm failure; the full 2000ms+5000ms chain budget is test-pinned against the 10s guard budget. ALSO writes an EVALUATION stream, `.minsky/retrospective-trigger-evaluations.jsonl` (every evaluated turn, fired or not — the mt#3583 pattern; separate file because coverage-receipt reads calibration-record existence as fire-evidence), closing the gap where a Rung-1 miss wrote nothing and recall misses were only ever human-discovered. `MINSKY_ACK_RETROSPECTIVE_TRIGGER`; Rung-3 kill switch `MINSKY_DISABLE_RUNG3_CONFIRM`.
