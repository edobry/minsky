...
- **Operator deferral** — an ACTION deferred to the principal without a same-turn capability probe. Sibling of ask-routing-deferral (which covers a DECISION). Calibration-first (mt#2459). FIVE surfaces: capability-deferral prose, `AskUserQuestion` option labels, permission-deferral prose (mt#3463), denial-anchored (mt#3533) — a permission-denied `tool_result` with no same-turn retry in a different command shape, so ADR-024's ladder governs only its phrase half — and ask-justification (mt#3999): an operator-routed `asks_create` whose justification asserts a named capability does not exist, in a turn consulting FEWER THAN TWO distinct channels. That conjunct is channel COUNT, not probe presence — the anchor instance DID probe, and the one channel it read is what lied; `routingTarget` comes from the RESULT, and a policy-closed ask is excluded. Writes an evaluation stream over all five, fired or not, so the miss RATE is measurable; its `evaluated` field marks two denominators — group by it. Mechanism, the mem#276 carve-out + its SYNTHETIC control, surface D's threshold: `docs/architecture/hooks/operator-deferral-detector.md`. `MINSKY_SKIP_OPERATOR_DEFERRAL`.
