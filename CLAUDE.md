...
- **Retrospective-trigger** — reminds `/retrospective`; Stop sibling `turn-end-retro-scan`. Full ADR-024 ladder as of mt#3652 (Rung-2 log-only, Rung-3 confirm injects). Its Stop↔prompt dedup keys off the resolver that produced the scanned text (mt#3950); a positional key named the previous turn and double-injected. `MINSKY_ACK_RETROSPECTIVE_TRIGGER`; Rung-3 kill switch `MINSKY_DISABLE_RUNG3_CONFIRM`.
...
- **Turn-end-bare-ref-scan** — Stop scan (mt#3286): the closing message carries an entity ref the reader cannot click. Posture is per finding class and tracks the COMPLEMENT of what the display linkifier repairs: `bare-short-id` (`ask#N`/`mem#N`/`ws#N`), `malformed-target` and `raw-uuid-label` are LIVE; `bare-ref` (`mt#N` / `PR #N`) is RECORD-ONLY. Since mt#3960 the complement is computed PER FINDING against mt#3914's short-id→UUID map, so an unmapped short id still flags and a missing map flags everything (fail-to-Rung-1). Its advisory is chain-capped at one follow-up (mt#3860/mt#3937). Retiring `bare-short-id` is an operator decision; mt#3947 holds the fire data. Narration, posture history and measurements: `docs/architecture/hooks/turn-end-bare-ref-scan.md`. `MINSKY_ACK_BARE_ENTITY_REF`.
...
- **Operator deferral** — an ACTION deferred to the principal without a same-turn capability probe. Sibling of ask-routing-deferral (which covers a DECISION). Calibration-first (mt#2459). FIVE surfaces: capability-deferral prose, `AskUserQuestion` option labels, permission-deferral prose (mt#3463), denial-anchored (mt#3533) — a permission-denied `tool_result` with no same-turn retry in a different command shape, so ADR-024's ladder governs only its phrase half — and ask-justification (mt#3999): an operator-routed `asks_create` whose justification asserts a named capability does not exist, in a turn consulting FEWER THAN TWO distinct channels — channel COUNT, not probe presence, because the anchor instance DID probe and that one channel is what lied. Evaluation stream over all five, fired or not, so the miss RATE is measurable; group by `evaluated`. Mechanism, carve-outs, thresholds: `docs/architecture/hooks/operator-deferral-detector.md`. `MINSKY_SKIP_OPERATOR_DEFERRAL`.
