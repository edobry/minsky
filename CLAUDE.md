...
- **Probe before deferring (mt#1819).** Before writing any of the phrases below in a PR body, spec `## Outcome` section, ask, status update, or chat — run a **tooling probe** to verify you actually lack the access you're about to claim is missing. Cost of a probe: ~30 seconds. Cost of a wrong deferral: 5–30 minutes of user attention plus a re-engagement cycle.
  ...
  **Trigger phrases** (any of these fires the probe requirement):
  - "deferred to operator" / "deferred to user"
  - "requires `<X>` access" (for a CLI, hosted service, account, secret store, etc.)
  ...
  **If any probe returns "tooling is available"**, do the work instead of writing the deferral.

  **If all probes fail**, state the probe results inline so the deferral has visible justification: e.g., `"Probed: which gh → not on PATH; no GitHub-org-admin skill; no scripts/gh-admin/; no memory matches. Deferred — requires user with GitHub org-admin access."` Without those results stated, the deferral is unjustified.
