...
**Normal merge path for bot-authored PRs:** `minsky-reviewer[bot]` (id 278512143) is a SEPARATE GitHub App identity from `minsky-ai[bot]` (id 277592531). Since mt#1073 (two-App identity) and mt#1656 (verification-mode preamble), the reviewer bot CAN and DOES submit APPROVE on bot-authored PRs. After reviewer-bot APPROVE, call `session_pr_merge` — the standard merge path that atomically sets DONE. The `gh api PUT /merge` bypass is a convergence-failure fallback only (CoT leakage, self-reversal, reviewer silence); it is NOT the default for bot-authored PRs. Production evidence: PRs #1256, #1258 (2026-05-24), #1282 (2026-05-25).
