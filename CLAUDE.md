...
- **CLI-substitutes-MCP** (mt#4144) — a `Bash`/`session_exec` invoking the Minsky CLI for a command that HAS an `mcp__minsky__*` equivalent, when MCP has not succeeded — or, since mt#4353, from the SECOND such call after the last success. INJECTS (never denied; "calibration-first" here was wrong for its whole life, mt#4290). `MINSKY_ALLOW_CLI_SUBSTITUTION`. Detail: `cli-mcp-substitution-detector.md`.
