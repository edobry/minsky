...
2. Reads `transcript_path` and `parseTranscript()`s the FULL session transcript,
   then scans **all** lines (via `findToolUseInputs` in
   `.claude/hooks/transcript.ts`) for a `mcp__minsky__tasks_spec_get` — or a
   `mcp__minsky__tasks_get` with `includeSpec: true` — tool_use whose `taskId`
   matches the target.
3. If no spec-surfacing call for the target exists anywhere in the session, the
   call is denied; otherwise it passes.
