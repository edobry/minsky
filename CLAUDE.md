...
- **Guard-events sweep ingest** (mt#4035) — SessionEnd hook runs one `guard-events.ingest` sweep
  tick over the 40-stream guard/calibration exhaust (per-stream HWM cursor, sha256 dedupe-key
  batched insert into `guard_events`). Latency optimization only — the cockpit daemon's periodic
  `startGuardEventsSweepBackstop` (`src/cockpit/sweepers.ts`) is THE CORRECTNESS LAYER, since
  SessionEnd does not reliably fire (ADR-017/mt#2313). `MINSKY_SKIP_GUARD_EVENTS_INGEST_HOOK`.
