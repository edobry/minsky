...
block is capped at `MERGED_CONTEXT_BUDGET_CHARS` (`.minsky/hooks/dispatcher.ts`), derived from the
registry's own `attentionCost` annotations — read the constant rather than a figure quoted here,
which goes stale every time the corpus is trimmed (it did within a day of mt#3479's re-derivation). Over budget, the lowest-priority fragments are dropped and NAMED in
