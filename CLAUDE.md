...
- **Operator deferral** — an ACTION deferred to the principal without a same-turn capability probe. Sibling of ask-routing-deferral (which covers a DECISION). Calibration-first (mt#2459). Four surfaces: capability-deferral prose ("I can't"), AskUserQuestion option labels, **permission-deferral prose ("I can, shall I?" — mt#3463)**, which excludes genuinely destructive or principal-reserved actions because there the ask is CORRECT, and **denial-anchored (mt#3533)** — an escalation or deferral resting on a permission-denied `tool_result` with no same-turn retry in a different COMMAND SHAPE (leading token differs, or compound became simple). That anchor is a structured result rather than a phrase, which is the point: the prose is third-person about a tool ("the API was denied") and four recorded instances each worded it differently, so no corpus reaches them — ADR-024's ladder governs the phrase half of the trigger, not the denial half. A denial whose reason names a security concern never fires (mem#276's stop-and-escalate carve-out); that control is SYNTHETIC, because zero of the 73 denials in the local corpus carried one. Writes an evaluation stream (`.minsky/operator-deferral-evaluations.jsonl`) covering ALL FOUR surfaces, fired or not, so the miss RATE is measurable — the input ADR-024's rung decisions need and a fire-only log cannot give. Records carry `evaluated: "prose-turn" | "ask-tool-call"`; the two are different denominators (per completed turn vs per `AskUserQuestion` call), so group by it rather than pooling. Graduation threshold for the denial-anchored surface: a `/calibration-review` pass over >=10 classified fires, per ADR-024's ladder. `MINSKY_SKIP_OPERATOR_DEFERRAL`.
...
- **Duplicate-check search provenance** (mt#4004) — a `tasks_create` whose duplicate-check record CLAIMS a search (past tense, or naming `tasks_search`/`tasks_similar`) in a session where no such call appears in the transcript. Third guard on that record and the one closing the gap between the other two: the deny-tier sibling checks the line is PRESENT, the signature scan checks its VERDICTS are true, neither asks whether the search RAN. Incident mt#4003 — a fabricated search narrative filed against four tasks that already owned the work, one filed 84 minutes earlier by another actor; the signature scan caught it only because the false provenance happened to coincide with wrong verdicts. The sanctioned `Duplicate check: no candidates found.` never fires (it reports an outcome, not an action), and an absent transcript records `skipped`, never `clean`. Calibration-first. `MINSKY_SKIP_SEARCH_PROVENANCE`.
