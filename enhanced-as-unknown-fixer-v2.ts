...
class EnhancedAsUnknownFixerV2 {
  ...
  async execute(): Promise<void> {
    ...
    const sourceFiles = this.project
      ...
      .filter(
        (file) =>
          !file.getFilePath().includes(".d.ts") && !file.getFilePath().includes("node_modules")
      );
  ...
  private isSafeObjectMethodArg(exprText: string): boolean {
    return (
      /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(exprText) ||
      /^this\.[a-zA-Z_$][a-zA-Z0-9_$.]*$/.test(exprText) ||
      ["data", "result", "options", "config", "params", "state", "obj", "object", "item"].includes(
        exprText
      )
    );
  }
  ...
  private isSafeFunctionParameter(exprText: string): boolean {
    return (
      /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(exprText) ||
      ["params", "args", "options", "config", "data", "context", "request", "response"].includes(
        exprText
      )
    );
  }
