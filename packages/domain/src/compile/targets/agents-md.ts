 ...
 *
 * Imported rather than re-declared as of mt#4986 — see the sibling constant in
 * `claude-md.ts` for why.
 */
const AGENTS_MD_BANNER = MONOLITHIC_GENERATED_BANNER;
...
function makeAgentsMdTarget(
  ...
  return {
    ...
    async listOutputFiles(
      ...
      // mt#4986 SC4 — see `claude-md.ts`'s `listOutputFiles` for the rationale.
    ...
    async compile(
      ...
      if (foreignSkip !== undefined) {
        return {
          target: "agents.md",
          filesWritten: [],
          // Nothing was included — see the sibling guard in `claude-md.ts`.
          definitionsIncluded: [],
