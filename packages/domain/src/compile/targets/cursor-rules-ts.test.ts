...
import {
  makeCursorRulesTsTarget,
  buildRuleMdc,
  hasBannerAtEmittedPosition,
...
// ─── mt#573 SC4 / PR #3651 R1: which files orphan removal may delete ────────
describe("hasBannerAtEmittedPosition — banner POSITION, not mere presence", () => {
  const BANNER = "# Generated by minsky rules compile. Do not edit directly.";

  it("accepts the shape this target emits — banner on line 2, under the --- opener", () => {
    expect(hasBannerAtEmittedPosition(`---\n${BANNER}\nname: x\n---\nbody`)).toBe(true);
  });

  // The false positive the reviewer named. A rule ABOUT generated files is
  // exactly the kind a user hand-writes, and a substring check would delete it
  // for quoting the thing it documents.
  it("REFUSES a hand-authored rule that merely quotes the banner in its body", () => {
    const handAuthored = `---\nname: my-rule\n---\nOur outputs start with:\n\n    ${BANNER}\n`;
    expect(hasBannerAtEmittedPosition(handAuthored)).toBe(false);
  });

  it("REFUSES a file whose first line is not the frontmatter opener", () => {
    expect(hasBannerAtEmittedPosition(`${BANNER}\n---\nname: x\n`)).toBe(false);
  });

  it("tolerates trailing whitespace an editor may add", () => {
    expect(hasBannerAtEmittedPosition(`---  \n${BANNER}  \nname: x\n`)).toBe(true);
  });

  it("REFUSES an empty file", () => {
    expect(hasBannerAtEmittedPosition("")).toBe(false);
  });
});
