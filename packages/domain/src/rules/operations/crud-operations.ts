...
/**
 * Probe which legacy `rules compile` targets a bare (no `--target`) invocation
 * should regenerate — RETIRED by the mt#3058 cutover, always returns `[]`.
 *
 * Its three former targets — agents.md, claude.md, claude-rules — moved to the
 * new `compile` pipeline (`minskyCompileTargetsFromPresence`,
 * packages/domain/src/compile/compile.ts), which is now the sole bare-invocation
 * writer of CLAUDE.md / AGENTS.md / .claude/rules. Banner-based on-disk
 * detection could NOT be used to disable this probe, because the new pipeline
 * emits the identical `<!-- Generated by minsky rules compile. -->` banner
 * during coexistence — a banner check would still match all three. So the probe
 * is reduced to `[]`, and `compileRules` no longer falls back to a default
 * `agents.md` target (which would re-introduce a legacy write of AGENTS.md).
 *
 * Explicit `rules compile --target agents.md|claude.md|claude-rules` still works
 * (it bypasses this probe) until mt#2996 removes the legacy command entirely.
 * The signature is retained for callers/tests. Exported for unit testing.
 */
...
export async function compileRules(
  ...
  // mt#3058 cutover: a bare (no `--target`) `rules compile` regenerates NOTHING.
  // probeLegacyCompileTargets always returns [] now — its former targets
  // (agents.md / claude.md / claude-rules) are owned by the new `compile`
  // pipeline — and the OLD fallback to a default ["agents.md"] would have
  // re-introduced a legacy write of AGENTS.md, re-creating the dual-write the
  // cutover removes. Explicit `--target` still compiles via the early return
  // above (until mt#2996 removes the legacy command).
  ...
  if (probedTargets.length === 0) {
    return {
      ...
      target: "",
      check: options.check,
      stale: options.check ? false : undefined,
  ...
  // Defensive: the probe is contractually empty post-cutover, but if a future
  // change re-populates it, compile each probed target rather than silently
  // dropping them.
  ...
  for (const targetId of probedTargets) {
