 ...
 * - "claude-rules": .claude/rules/ exists with at least one .md file
 *   (mt#2868; mirrors the cursor-rules presence-only check above rather than
 *   reading file content — consistent with how bare-invocation probing
 *   already treats the sibling multi-file target)
 *
...
export async function probeLegacyCompileTargets(
  ...
  try {
    ...
    // Banner-gate the detection (reviewer R1): `.claude/rules/` legitimately
    // holds hand-authored rule files too — only a file WE generated (carries
    // the generation banner) makes this a compile target.
    for (const entry of entries) {
      if (!entry.endsWith(".md")) continue;
      try {
        const content = String(await fs.readFile(path.join(rulesDir, entry), "utf-8"));
        if (content.includes("<!-- Generated by minsky rules compile. Do not edit directly. -->")) {
...
async function compileSingleTarget(
  ...
  if (options.check) {
    ...
    // For cursor-rules / claude-rules, build the per-file content map once
    // outside the loop
    ...
    let claudeRulesFilesMap: Map<string, string> | undefined;
    if (targetId === "claude-rules") {
      const { buildClaudeRulesContent } = await import("../compile/targets/claude-rules");
      const outputDir = options.output || target.defaultOutputPath(options.workspacePath);
      const { files } = buildClaudeRulesContent(allRules, outputDir);
      claudeRulesFilesMap = new Map(files.map((f) => [f.path, f.content]));
    }
    ...
    for (const filePath of expectedFiles) {
      ...
      // For cursor-rules / claude-rules (multi-file), use the pre-built
      // per-file content map.
      let expectedContent: string | undefined;
      if (targetId === "cursor-rules") {
        expectedContent = cursorFilesMap?.get(filePath);
      } else if (targetId === "claude-rules") {
        expectedContent = claudeRulesFilesMap?.get(filePath);
      } else {
    ...
    // For claude-rules, detect orphan .md files: a file left on disk whose
    // source rule lost eligibility (globs cleared, alwaysApply flipped, or
    // disabled via selection config) is stale in --check mode the same way
    // `compile()` itself would remove it (mt#2868 design decision 1 — a
    // stray glob-less-looking leftover is exactly the always-loaded-budget
    // hazard this target exists to avoid). Only files carrying the
    // generation banner are treated as orphans; hand-authored files in
    // `.claude/rules/` are left alone.
    if (!staleFile && targetId === "claude-rules") {
      const outputDir = options.output || target.defaultOutputPath(options.workspacePath);
      try {
        const entries = await fs.readdir(outputDir);
        const expectedSet = new Set(expectedFiles.map((f) => path.basename(f)));
        const { CLAUDE_RULES_BANNER } = await import("../compile/targets/claude-rules");

        for (const entry of entries) {
          if (!entry.endsWith(".md") || expectedSet.has(entry)) continue;
          const entryPath = path.join(outputDir, entry);
          try {
            const content = String(await fs.readFile(entryPath, "utf-8"));
            if (content.includes(CLAUDE_RULES_BANNER)) {
              staleFile = entryPath;
              break;
            }
          } catch {
            // Unreadable — not a staleness signal we can act on
          }
        }
      } catch {
        // Directory doesn't exist — already handled above via expectedFiles check
