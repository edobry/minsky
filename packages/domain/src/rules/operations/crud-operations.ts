 ...
 *   (mt#2868; mirrors the legacy cursor-rules presence-only check that used
 *   to live here rather than reading file content — consistent with how
 *   bare-invocation probing already treats the sibling multi-file target)
 *
...
async function compileSingleTarget(
  ...
  if (options.check) {
    ...
    // For claude-rules, build the per-file content map once outside the loop
    ...
    for (const filePath of expectedFiles) {
      ...
      // For claude-rules (multi-file), use the pre-built per-file content map.
