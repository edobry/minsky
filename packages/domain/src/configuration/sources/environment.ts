...
export const HOOK_ONLY_ENV_VAR_CATEGORIES: Readonly<Record<string, HookOnlyEnvVarCategory>> = {
  ...
  MINSKY_SKIP_CONSUMER_ACCOUNT: "operator-override", // .claude/hooks/consumer-account-evidence.ts (mt#4493) — override for the calibration-first consumer-account check (log-only): a diff removing a signal-producing call should name what CONSUMED it. Fifth sibling, separate for the same reason. Unlike the four above, its trigger reads diff HUNKS rather than the file list, so it is the only one that pays for a second `gh api` call — prefiltered to PRs touching a scanned root.
