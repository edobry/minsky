...
export const HOOK_ONLY_ENV_VAR_CATEGORIES: Readonly<Record<string, HookOnlyEnvVarCategory>> = {
  ...
  MINSKY_DISABLE_STALE_STATE_RUNG2: "operator-override", // .claude/hooks/turn-end-stale-state-assertion-scan.ts (mt#4580) — the same kill switch for THAT guard's Rung-2 stage; per-guard rather than shared, so one detector's semantic stage can be silenced without silencing the other's. Rung 1 keeps running
