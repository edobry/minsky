...
/*
 * The four SHADOWED kinds (mt#4875). Each of these carries `isMeta: true` AND
 * names its own `origin.kind`, and before mt#4875 the `isMeta` check ran first
 * and unconditionally — so every one of them was stored as `harness_meta` and
 * `origin.kind` was unreachable. Shapes read off `~/.claude/projects/**` on
 * 2026-09-01; `origin` key sets are verbatim, message text is elided.
 */

/** A cross-session peer message. `origin` carries the whole structured envelope. */
const peerLine = {
  type: "user",
  isMeta: true,
  origin: {
...
/** A coordinator message injected mid-turn. */
...
/**
 * A task notification that ALSO carries `isMeta`. Distinct from
 * {@link taskNotificationLine} above, which carries `origin` and no `isMeta` —
 * both shapes occur, and only this one was shadowed (96 of the corpus's
 * task-notification lines; the rest reach `origin.kind` either way).
 */
const metaTaskNotificationLine = {
  type: "user",
  isMeta: true,
  origin: { kind: "task-notification" },
...
/** The line the harness injects when a usage limit resets and work resumes. */
const autoContinuationLine = {
  type: "user",
  isMeta: true,
...
/**
 * The ONE shape where `origin.kind` and `isMeta` disagree and `isMeta` is right
 * (mt#4875). The operator ran a `!` command — hence `origin: {kind: "human"}` —
 * and the harness then wrote this caveat wrapper itself, hence `isMeta`. The
 * text is the harness's, so the line is NOT operator speech. Verbatim content;
 * this is the negative control on the carve-out, and it must fail if the
 * carve-out is dropped.
 */
const localCommandCaveatLine = {
  parentUuid: "0f39dcbe-1a2e-4bd8-9b6a-2c9c5c6f3a11",
  isSidechain: false,
  type: "user",
  isMeta: true,
  origin: { kind: "human" },
  ...
  slug: "pure-spinning-mitten",
  timestamp: "2026-07-23T18:58:46.641Z",
  userType: "external",
  entrypoint: "cli",
};
...
describe("classifyUserLineOrigin — precedence", () => {
  ...
  // mt#4875 — this test's name used to read "isMeta wins over origin.kind",
  // generalizing from its ONLY case (`human`) to every kind. The behaviour it
  // pins is right; the name was what made the shadowing of the other four kinds
  // look intended. Name corrected, subject unchanged.
  test("isMeta wins over origin.kind ONLY when the kind is human", () => {
  ...
  test.each([
    ["peer", peerLine, "peer"],
    ["coordinator", coordinatorLine, "coordinator"],
    ["task-notification", metaTaskNotificationLine, "task_notification"],
    ["auto-continuation", autoContinuationLine, "auto_continuation"],
  ])("origin.kind %s outranks isMeta", (_label, line, expected) => {
    // Before mt#4875 every one of these returned HARNESS_META_ORIGIN, because
    // `isMeta` was checked first and unconditionally. Reverting that ordering
    // fails all four.
    expect(classifyUserLineOrigin(line)).toBe(expected);
  });

  test("the local-command caveat stays harness_meta even though its kind is human", () => {
    // The negative control on the carve-out (mt#4875 AT2). `origin.kind` names
    // who CAUSED the line — the operator, who ran a `!` command — while the TEXT
    // is a wrapper the harness wrote. Moving `origin.kind` above `isMeta`
    // WITHOUT the human carve-out turns this green-to-red, which is the point:
    // it would file harness boilerplate as the operator's own words.
    expect(classifyUserLineOrigin(localCommandCaveatLine)).toBe(HARNESS_META_ORIGIN);
    expect(isOperatorAuthored(localCommandCaveatLine)).toBe(false);
  });

  test("origin.kind human WITHOUT isMeta is still the operator", () => {
    // The carve-out is conditioned on `isMeta`, not on the kind alone — an
    // ordinary typed prompt must not be swept up by it.
    expect(classifyUserLineOrigin(operatorLine)).toBe(OPERATOR_ORIGIN);
  });
