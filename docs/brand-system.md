...
- **This obligation is enforced, not advisory (mt#3411).** `src/design-system/brand-token-drift.ts` parses this table and asserts that `src/cockpit/web/index.css`, `services/site/src/styles/global.css`, and that file's `@supports` hex fallback all match it. Editing a value here without re-deriving the surfaces fails the test suite, naming the token and the surface. Adding a row also requires a `TOKEN_MAP` entry in that module, or the check fails rather than silently skipping the new token. See `docs/design-system.md` §7 for the full contract.
...
- **Hex is the surface format; OKLCH is the canonical token format.** Render values in OKLCH for any new design system (`oklch()` is supported in modern browsers and gives perceptually-uniform tuning). Cockpit ships OKLCH-native tokens directly in `src/cockpit/web/index.css` (migrated from HSL under mt#1935 — see §7 for the current mapping).
...
| ------------------------------ | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| **Cyberbrain**                 | Ghost in the Shell           | Category name. Foreground tagline form: _"The cyberbrain for software organizations."_ (bare form; the earlier qualifier was retired 2026-05-31 — see `minsky-brand` SKILL.md §7a; provisional pending mt#2046) | Don't pluralize as "cyberbrains" in marketing copy.                               |
...
### Cockpit token mapping (post-mt#1935; current reality, refreshed mt#2915)

Cockpit ships OKLCH-native custom properties directly in `src/cockpit/web/index.css` — no HSL survives in the stylesheet. The `.dark` block is the shipped, authoritative theme (cockpit is dark-first via `<html class="dark">`; the `:root` light block is placeholder-only). Values below are OKLCH `L C H` triplets from the `.dark` block, matching this doc's §2 canonical format:

| Cockpit var (`.dark`)                       | Brand token                               | Value                                                                      | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------- | ----------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--background`                              | `bg.base`                                 | `0.078 0 0`                                                                | Exact match.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--foreground`                              | `text.primary`                            | `0.946 0 0`                                                                | Exact match.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--card`                                    | `bg.elevated`                             | `0.155 0 0`                                                                | Exact match.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--muted-foreground`                        | `text.muted`                              | `0.717 0 0`                                                                | Exact match.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--text-subtle`                             | `text.subtle`                             | `0.517 0 0`                                                                | Exact match; standalone token, not a shadcn slot.                                                                                                                                                                                                                                                                                                                                                                                                |
| `--primary`                                 | `signal.cyan`                             | `0.745 0.124 215`                                                          | Cockpit's primary button/active-state color IS the brand accent — resolves the pre-migration "reassess" note.                                                                                                                                                                                                                                                                                                                                    |
| `--signal-cyan` / `--signal-cyan-dim`       | `signal.cyan` / `signal.cyan.dim`         | `0.745 0.124 215` / `0.560 0.092 220`                                      | Exact match; exposed standalone for non-button accent uses (active-status indicators, sync-gauge instruments).                                                                                                                                                                                                                                                                                                                                   |
| `--warn-amber`                              | `warn.amber`                              | `0.756 0.180 70`                                                           | Exact match.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--warn-red` / `--destructive`              | `warn.red`                                | `0.585 0.219 27`                                                           | `--destructive` is the shadcn-slot alias; both resolve to the same value.                                                                                                                                                                                                                                                                                                                                                                        |
| `--iso-pastel`                              | `iso.pastel`                              | `0.911 0.030 75`                                                           | Exact match.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `--muted` / `--secondary` / `--accent`      | (cockpit-local neutrals)                  | `0.180` / `0.240` / `0.300` (`0 0`)                                        | Not brand-palette tokens — cockpit-local neutral-surface steps for badge/pill/rollup fills. Differentiated by lightness only under mt#2915 (`docs/design-system.md` §5.3) — previously identical (`0.200 0 0` each), collapsing TODO/PLANNING/IN-PROGRESS badges to the same gray.                                                                                                                                                               |
| `--liveness-healthy/-idle/-stale/-orphaned` | (cockpit-local, no brand `success` token) | `0.580 0.140 145` / `0.720 0.175 70` / `0.640 0.185 55` / `0.560 0.215 27` | Cockpit-local liveness sub-tokens; brand palette intentionally has no `success` (per cockpit-design skill). Recalibrated mt#2917: `stale` moved from red to a deeper amber (a simply-idle-a-while session is not a hard-alarm state per `docs/design-system.md` §5.1); `orphaned` (no live owner — the genuinely dead state) took the red slot instead. Previously inverted (stale rendered red at volume; orphaned rendered a muted blue-gray). |
| `--vsm-s1` … `--vsm-learn`                  | (cockpit-local, plant board)              | See §VSM organ palette below                                               | Seven VSM-organ tokens (mt#2376), unrelated to the shadcn/status token families above.                                                                                                                                                                                                                                                                                                                                                           |
...
- **Cockpit color tokens:** `src/cockpit/web/index.css` (OKLCH-native since mt#1935; see §7 for the current mapping).
