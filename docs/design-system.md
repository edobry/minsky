...
| Select            | `ui/select.tsx` (Radix, mt#3347)                                                                                                                                                                                           | trigger: `hover:bg-muted/70`                | trigger: `focus:ring-1 ring-ring`; item: `focus:bg-accent focus:text-accent-foreground` | item: `data-[state=checked]` renders a `Check` indicator; trigger shows the selected label via `SelectValue` | trigger: `disabled:opacity-50 disabled:cursor-not-allowed`; item: `data-[disabled]:opacity-50 data-[disabled]:pointer-events-none` | n/a — options are resolved before the panel opens. Disable the trigger while a dependent mutation is pending (see `TaskDetail.tsx`'s model picker).     |
