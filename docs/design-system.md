...
| Scrollbar          | `index.css` `@layer base` (mt#3701) — native scrollbar styled from tokens, cockpit-wide; `.scrollbar-none` is the opt-out                                                                                                           | thumb `--border` → `--muted-foreground`                                                  | n/a — not focusable                                                                                                                 | thumb `--muted-foreground` while dragging                                                                                                                                                                                                                                        | n/a                                                                                                                                | n/a                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
**Scrollbar decision (mt#3701).** The native scrollbar was the last piece of cockpit chrome drawn
by the platform rather than by this system — an OS-gray bar inside a near-black rail. It is now
styled once in `index.css`'s base layer from `--border` (rest) and `--muted-foreground` (hover),
thin, with a transparent track and no stepper buttons, applying to every scroll surface rather
than per-container. Two consequences worth knowing before changing it:
...
- **Styling `::-webkit-scrollbar` opts out of macOS overlay scrollbars**, so the gutter takes
  layout width (~10px) on every scroll container. Accepted: it also means content stops reflowing
  when a scrollbar appears. `.scrollbar-none` (TabBar) still suppresses everything — a class
