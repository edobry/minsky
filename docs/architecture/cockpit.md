...
| `/proposals`               | Proposals           | EngProd toil-miner curation gate (mt#3331): filed `engprod-proposal` tasks grouped by mining run, with evidence + Accept (tag swap to `engprod-accepted`, mt#5130) / Reject (CLOSED) — task row + ledger verdict, atomically — see `src/cockpit/routes/engprod-proposals.ts`)                                                         |
