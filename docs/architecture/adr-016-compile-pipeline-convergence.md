...
### Phase 4 shipped (mt#2993, 2026-09-11)

Phase 4's "consolidate the two pre-commit staleness checks into one" is complete. The cutover (mt#3058) had already moved `claude.md`, `agents.md` and `claude-rules` onto the new-pipeline `runCompileCheck` and reduced the legacy `runRulesCompileCheck` to a no-op; mt#2993 deleted that remainder, its Step-9 registration, and its `rules-compile-check` interceptor entries, and repointed its classifier tests (`src/hooks/compile-check.test.ts`) at the `[compile --check]` marker prefix. `src/hooks/pre-commit.ts` now runs exactly ONE compile-staleness step, over all seven targets (`claude-skills`, `cursor-rules-ts`, `claude-agents`, `claude-hooks`, `claude.md`, `agents.md`, `claude-rules`), carrying the `MINSKY_SKIP_SIZE_BUDGET` override. The `claude-agents` drift named above was reconciled by mt#2497, which subsumed mt#1654. "Both pre-commit checks run" in the coexistence bullet above is therefore historical; the `rules compile` COMMAND still exists until phase 5 (mt#2996).
