...
- **PR [#831](https://github.com/edobry/minsky/pull/831)** (mt#1337) — multi-line review comments — **MERGED** (squash-merge SHA `8d45836a5`). Initial review flagged a side-defaulting hole (caller passing `startLine` and `startSide: LEFT` with no `side` produced a mismatched payload that GitHub would reject); fix landed in commit `2416acde5` to infer `side` from `startSide` when only one is provided. Subsequent commits added a GitHub API doc citation and documented the LEFT-side requirement for deletions. Bot re-review unobtainable due to chain-of-thought leakage class (mt#1382); merge proceeded via `gh api PUT` bypass after stale review dismissal and a self-review. Documented in `feedback_self_authored_pr_merge_constraints` and `feedback_gh_api_bypass`.
