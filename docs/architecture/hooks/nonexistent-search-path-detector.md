...
| Relative path, `Bash`, after a `cd` to a LITERAL directory   | that directory                | **Checked** there (mt#5111). `cd packages && grep … src/foo` resolves `src/foo` under `packages`; a `..` walks back; `pushd` counts as a `cd`.                                                                                                                                                                                                                                                                                                                                                 |
| Relative path, `Bash`, after a `cd` this guard cannot follow | unknown                       | **Silent.** `cd "$T"`, `cd $(mktemp -d)`, a bare `cd`, `cd -`, and `popd` (no directory stack is modelled): every LATER relative operand is unresolvable. Statements BEFORE the cd still resolve against `input.cwd`, as they do in the shell. Until mt#5111 one `cd` anywhere silenced the whole command.                                                                                                                                                                                     |
| A path an EARLIER statement of the same command PRODUCES     | as resolved at that statement | **Silent**, counted under `producedCount` (mt#5111). A redirect target (`> f`, `>> f`, `2> f`, `&> f`), a `tee` operand, a `mkdir` directory or anything under one. The stat runs before the command, so the file is not there YET — 24 of 42 fires in the 2026-09-11 → 09-14 window were a run redirected into a log a later statement grepped. The join is the resolved absolute path, so a relative target written before a `cd` and searched after it is a different file and still fires. |
...
asking about recall rather than precision. A produced-path skip increments `producedCount` instead
(mt#5111), on the same records, for the same reason.

Replayed over the 41 fires between the mt#5111 filing (2026-09-11T18:25Z) and its fix, with the
reported segment treated as absent: the pre-fix walk re-fires 38 and the post-fix walk 14. The 24
silenced are all produced-shape names (`mt2993-hooks*.log`, `mt5080-*`, `mt5131-tc*.log`,
`pr3412-files.txt` ×3, …); the 14 that still fire are bare filenames grepped from a directory that
does not hold them, plus two tokenizer artifacts (`)`, ` Support`) that are not this class.
...
- `VALUE_TAKING_LONG_OPTS` is deliberately generous across GNU grep, BSD grep, ugrep and ripgrep. An
  ...
  tests.
- **A statement boundary is `;`, `&&`, `||`, AND a newline (mt#5076); a single `|` is a pipeline
  STAGE boundary inside a statement (`splitPipeline`), and each stage is scanned on its own.** Until
  mt#5076 the guard split statements with `splitTopLevel`, which honours the first three only, so a
  multi-line command's second line was read as the tail of the first line's `grep`: `grep -c 'x' "$F"\necho '=== compact
