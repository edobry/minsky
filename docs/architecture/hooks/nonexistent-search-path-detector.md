...
| Relative path, `Bash`, after a `cd` this guard cannot follow | unknown                       | **Silent.** `cd "$T"`, `cd $(mktemp -d)`, a bare `cd`, `cd -`: every LATER relative operand is unresolvable. Statements BEFORE the cd still resolve against `input.cwd`, as they do in the shell. Until mt#5111 one `cd` anywhere silenced the whole command.                                                                                                                                                                                                                                  |
