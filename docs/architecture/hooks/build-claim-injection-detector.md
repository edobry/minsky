...
| (c) no rebuild evidence | NO `deployment_wait-for-latest`/`status`/`logs` tool call, and no Bash/`session_exec` command matching `install-local.sh`, `tauri build`/`dev`, `cargo build`, `npm run build`, `bun run build`/`dev`, or `railway up`, anywhere in the session                                                                        |
