...
| policy-coverage (retired)         | `.minsky/policy-coverage-calibration.jsonl`                                                                          | RETIRED 2026-08-16 (mt#4197) — producer deleted; log retained as evidence (declared via RETIRED_CALIBRATION_PRODUCERS, mt#4204) | `{reason, outcome, evidence?}` (not matched-phrase; diversity over `reason`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | registered-explicit + retired-producer (mt#4204)                                                                                                                | ingest                                             |
...
| spec-scope-execution              | `.minsky/spec-scope-execution-calibration.jsonl`                                                                     | `.minsky/hooks/spec-scope-execution-check.ts`                                                                                   | `outcome: "flagged" \| "clean" \| "skipped"`, `taskId`, `enumeratedCount`, `editedCount`, and on a flag `untouched: [{path, line}]` — the spec's OWN line beside each path, because whether that line was CONDITIONAL is the one thing separating a real finding from this check's dominant false positive. Every not-adjudicable path records `skipped` with a `reason`, never `clean` (mt#4544 SC5). NOTE for whoever next audits this table: the sibling `enumeration-scope` stream has no row here and should get one — its absence is a gap in this document, not a convention.                                                                                                                                                                                                                                                                                                                  | Registry-dispatched (`registry-pr-create-guards.ts`), recorder-only, `advisory` per ADR-042 §Posture                                                            | not yet ingested — calibration-first (mt#4544 SC6) |
