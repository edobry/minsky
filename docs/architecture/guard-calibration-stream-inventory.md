...
**Snapshot date:** 2026-08-12. The `.minsky/` JSONL count drifted from the
spec's observed 31 to 33 _during this same session_ — `negative-existence-claim
-calibration.jsonl` and `negative-existence-claim-evaluations.jsonl` appeared
when the mt#3918 detector (merged immediately before this task started) fired
live for the first time. Both are included below; the drift itself is evidence
this inventory needs to be a durable artifact, not a one-time count.
...
| execution-evidence-at-coverage    | `.minsky/execution-evidence-at-coverage-calibration.jsonl`                                                           | `.minsky/hooks/require-execution-evidence-before-merge.ts` (guard `require-execution-evidence-before-merge`, standalone canary) | matches-shape (generic)                                                                         | registered-derived (2-log guard)                     | ingest                                          |
| execution-evidence-test-first     | `.minsky/execution-evidence-test-first-calibration.jsonl`                                                            | same guard, via `test-first-evidence.ts` called in-process                                                                      | matches-shape (generic)                                                                         | registered-derived (2-log guard)                     | ingest                                          |
...
| retrospective-trigger             | `.minsky/retrospective-trigger-calibration.jsonl`                                                                    | `.minsky/hooks/retrospective-trigger-scanner.ts` + `turn-end-retro-scan.ts` (2 guards, 1 log)                                   | `matches: {family, phrase}[]`                                                                   | registered-explicit                                  | ingest                                          |
...
| wall-of-text                      | `.minsky/wall-of-text-calibration.jsonl`                                                                             | `.minsky/hooks/wall-of-text-detector.ts`                                                                                        | `{wordCount, trigger, leadLabelHits}` (not matched-phrase; diversity over `session_id`)         | registered-explicit                                  | ingest                                          |
...
```
$ find .minsky -maxdepth 1 -name "*.jsonl" | sort | wc -l
31   # 26 *-calibration.jsonl + 4 *-evaluations.jsonl + subagent-model-mismatch.jsonl,
     # matching the spec's 2026-08-12 baseline exactly, PLUS 2 files that appeared
     # mid-session (negative-existence-claim's first live fire) -> 33 total, all
     # accounted for in §A/§B above.
```
