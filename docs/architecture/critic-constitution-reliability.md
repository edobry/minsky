...
path) log a structured `console.error` event when `submitReview` throws,
...
`reviewer.submit_error_notice_failed` (CoT-error path). Both include
`prUrl`, `sha`, `primaryReason`, `submitError`, `provider`, and `model` so
the secondary failure is correlatable with the primary CoT-leak event.
