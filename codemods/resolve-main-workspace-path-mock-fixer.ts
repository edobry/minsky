...
export function fixDependencyMockInFile(sourceFile: SourceFile): DependencyMockFixResult {
  ...
  if (
    !content.includes("listTasksFromParams") &&
    !content.includes("getTaskFromParams") &&
    !content.includes("interface-agnostic")
  ) {
    return {
      filePath,
      changed: false,
  ...
  const mockFunction = usesBunMock
    ? 'mock(async () => "/test/workspace/path")'
    : 'vi.fn().mockResolvedValue("/test/workspace/path")';
      ...
      if (initializer instanceof ObjectLiteralExpression) {
        ...
        const hasDependencyMethods = properties.some((prop) => {
          if (prop instanceof PropertyAssignment) {
            ...
            return ["getTaskService", "resolveWorkspacePath", "createTaskService"].includes(
              propName
            );
          }
          return false;
