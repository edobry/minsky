...
export function fixGitServiceMockInFile(sourceFile: SourceFile): GitServiceMockFixResult {
  ...
  if (!content.includes('Clone Operations') && !content.includes('clone-operations') && !content.includes('gitService')) {
    return {
      filePath,
      changed: false,
  ...
  const mockFunction = usesBunMock ? 'createMock(async () => ({ success: true, message: "Repository cloned successfully" }))' : 'vi.fn().mockResolvedValue({ success: true, message: "Repository cloned successfully" })';
...
export function fixGitServiceCloneWithDependenciesMocks(testFiles: string[]): GitServiceMockFixResult[] {
