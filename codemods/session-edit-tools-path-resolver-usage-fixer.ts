...
export function fixSessionPathResolverUsage(sourceFile: SourceFile): PathResolverFixResult {
  ...
  for (const propAccess of propertyAccessExpressions) {
    ...
    if (objectName === "mockPathResolver") {
      if (propertyName === "resolvePath") {
        ...
        fixed = true;
        console.log(
          `✅ Replaced mockPathResolver.${propertyName} with mockValidatePath in ${filePath}`
        );
      }
    }
  }
  ...
  if (fixed) {
    ...
    return {
      ...
      changed: true,
      reason:
        "Updated SessionPathResolver usage to use module-level mocks instead of instantiation",
    };
  }
