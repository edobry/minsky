...
import { Project, Node, SyntaxKind, PropertyAssignment, StringLiteral, ObjectLiteralExpression } from "ts-morph";
...
export class TaskIdFormatNormalizer extends CodemodBase {
  ...
  description = "Normalize task ID formats in test mock data from display (#123) to storage (123) format";
  ...
  protected findIssues(): void {
    ...
    this.log(`Found ${this.taskIdIssues.length} task ID format issues across ${sourceFiles.length} files`);
  }
  ...
  protected processSourceFile(sourceFile: any): TaskIdFormatIssue[] {
    ...
    sourceFile.getDescendantsOfKind(SyntaxKind.ObjectLiteralExpression).forEach((objLiteral: ObjectLiteralExpression) => {
  ...
  protected fixIssue(sourceFile: any, issue: TaskIdFormatIssue): boolean {
    try {
      ...
      for (const stringLiteral of stringLiterals) {
        if (stringLiteral.getLiteralValue() === issue.taskIdValue &&
          ...
          this.log(`  Fixed task ID format: ${issue.original} → ${issue.suggested} in ${issue.file}`);
          return true;
        }
      }
  ...
  private fixSessionNamePatterns(sourceFile: any): number {
    ...
    sourceFile.getDescendantsOfKind(SyntaxKind.PropertyAssignment).forEach((propAssignment: PropertyAssignment) => {
      const propertyName = propAssignment.getName();
  ...
  public async run(): Promise<void> {
    ...
    console.log(`   Success rate: ${totalIssues > 0 ? Math.round((totalFixed / totalIssues) * 100) : 100}%`);
