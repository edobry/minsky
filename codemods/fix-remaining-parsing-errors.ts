...
function fixRemainingParsingErrors(content: string): { content: string; changes: number } {
  ...
  const patterns = [
    // Fix _() => { patterns (malformed arrow functions)
    {
      regex: /, _\(\) => \{/g,
      replacement: ', () => {',
      description: 'malformed arrow functions with _() =>'
    },
    // Fix function calls with underscore prefix like _()
    {
      regex: /", _\(\) => \{/g,
      replacement: '", () => {',
      description: 'function calls ending with _() =>'
    },
    ...
    {
      regex: /"([^"]*), (repository|file|directory|function|command)"/g,
      replacement: '"$1 $2"',
    ...
    // Fix malformed import statements with extra commas
    {
      ...
      description: 'import statements with trailing comma'
    },
