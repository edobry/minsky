...
export class RiskAwareTypeCastFixer extends CodemodBase {
  ...
  private initializeRiskPatterns(): void {
    this.riskPatterns = [
      ...
      },
      {
        ...
        riskLevel: "critical",
        filePatterns: ["**/*.ts"],
        codePatterns: [/\(fs\.statSync\([^)]+\) as any\)\.isDirectory/g, /\(fs\.[^)]+\) as any\)/g],
        replacement: (match: string, context: string) => {
      ...
      // HIGH RISK PATTERNS - Core Domain Logic
      {
        ...
        codePatterns: [/\([^)]+\) as any/g, /: any\b/g, /\bas any\b/g],
        replacement: ": unknown",
        requiresManualReview: true,
      },
      {
        ...
        codePatterns: [/\(task as any\)!/g, /\(taskData as any\)!/g],
        replacement: (match: string, context: string) => {
      ...
      // MEDIUM RISK PATTERNS - Infrastructure
      {
        ...
        codePatterns: [/\) as any\)\.version/g, /\(cli as any\)/g],
        replacement: (match: string, context: string) => {
      ...
      },
      {
        ...
        codePatterns: [/\(config as any\)/g, /\([^)]+Config as any\)/g],
      ...
      // LOW RISK PATTERNS - Test Infrastructure
      {
        ...
        codePatterns: [/\(mockFn as any\)/g, /\(expect as any\)/g, /\(bun\.expect as any\)/g],
        replacement: "as unknown",
        requiresManualReview: false,
      },
      {
        ...
        codePatterns: [/\([^)]+\) as any/g, /as any\b/g],
        replacement: "as unknown",
        requiresManualReview: false,
      },
    ];
  ...
  private processFileIssues(filePath: string, issues: TypeCastIssue[]): void {
    ...
    for (const issue of issues) {
      ...
      if (issue.requiresManualReview && !this.options.verbose) {
        this.log(
          `⏭️  Skipping manual review item: ${issue.description} in ${filePath}:${issue.line}`
        );
        continue;
      }
      ...
      if (issue.riskLevel === "critical" && !this.options.verbose) {
        this.log(
          `⚠️  Skipping critical risk item: ${issue.description} in ${filePath}:${issue.line}`
        );
        continue;
      }
  ...
  private printRiskBreakdown(): void {
    const breakdown = this.castIssues.reduce(
      (acc, issue) => {
    ...
    for (const [file, risks] of sortedFiles) {
      ...
      if (totalHighRisk > 0) {
        this.log(
          `   ${file}: ${risks.critical}C + ${risks.high}H + ${risks.medium}M + ${risks.low}L`
        );
      }
    }
  }
  ...
  protected generateReport(): void {
    ...
    const report = {
      ...
      riskBreakdown: this.castIssues.reduce(
        (acc, issue) => {
          acc[issue.riskLevel] = (acc[issue.riskLevel] || 0) + 1;
          return acc;
        },
      ...
      patternBreakdown: this.castIssues.reduce(
        (acc, issue) => {
