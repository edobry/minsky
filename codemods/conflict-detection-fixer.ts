...
export class ConflictDetectionFixer {
  ...
  public async fix(): Promise<void> {
    ...
    if (pattern1.test(content)) {
      content = content.replace(pattern1, "$1.mockImplementationOnce(() => Promise.resolve($2)); // $3");
      changed = true;
      console.log("✅ Fixed = mock( patterns");
    }
    
    // Pattern 2: Fix long single-line mock chains by splitting them
    const longChainPattern = /mockExecAsync = mock\([^)]+\)(\\.mockImplementationOnce\([^)]+\)){3,}/g;
    content = content.replace(longChainPattern, (match) => {
      ...
      if (parts.length > 1) {
        const result = parts[0] + "\n        " + 
          parts.slice(1).map(part => ".mockImplementationOnce" + part).join("\n        ");
        changed = true;
        return result;
      }
      return match;
