...
import { Project, SourceFile, SyntaxKind, ObjectLiteralExpression, PropertyAssignment, ShorthandPropertyAssignment, CallExpression, ArrowFunction, FunctionExpression, Block } from "ts-morph";
...
export function fixLogMockInFile(sourceFile: SourceFile): LogMockFixResult {
  ...
  if (content.includes('approveSession') || content.includes('Session Approve')) {
    ...
    const firstDescribe = callExpressions.find(call => 
      call.getExpression().getText() === 'describe'
    );
