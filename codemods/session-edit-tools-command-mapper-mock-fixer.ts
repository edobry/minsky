...
export function fixCommandMapperMockSignature(sourceFile: SourceFile): MockFixResult {
  ...
  for (const callExpr of callExpressions) {
    ...
    if (callExpr.getExpression().getKind() === SyntaxKind.PropertyAccessExpression) {
      const propAccess = callExpr.getExpression().asKindOrThrow(SyntaxKind.PropertyAccessExpression);
      
      // Check if this is commandMapper.addCommand.mockImplementation
      if (propAccess.getName() === "mockImplementation" &&
          propAccess.getExpression().getKind() === SyntaxKind.PropertyAccessExpression) {
        
        const innerPropAccess = propAccess.getExpression().asKindOrThrow(SyntaxKind.PropertyAccessExpression);
        ...
        if (innerPropAccess.getName() === "addCommand") {
          ...
          if (args.length === 1) {
            ...
            if (functionText.includes("(name: string, description: string, schema: any, handler: any)")) {
