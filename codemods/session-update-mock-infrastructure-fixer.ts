...
export function fixSessionUpdateMockInfrastructure(sourceFile: SourceFile): SessionUpdateMockFixResult {
    ...
    for (const declaration of declarations) {
      ...
      if (name === 'mockSessionProvider') {
        ...
        if (initializer && initializer.getKind() === SyntaxKind.ObjectLiteralExpression) {
          ...
          const existingMethods = objectLiteral.getProperties()
          ...
          if (existingMethods.length === 0) {
            ...
            fixed = true;
            console.log(`✅ Added missing getSessionWorkdir method to mockSessionProvider in ${filePath}`);
          }
        }
      }
    }
  ...
  if (!fixed) {
    ...
    if (match) {
      ...
      if (!objectContent.includes('getSessionWorkdir')) {
        ...
        fixed = true;
        console.log(`✅ Added missing getSessionWorkdir method to mockSessionProvider via text replacement in ${filePath}`);
      }
    }
  }
  ...
  if (fixed) {
    ...
    return {
      filePath,
      changed: true,
      reason: 'Added missing getSessionWorkdir method to mockSessionProvider - fixes sessionDB dependency issue'
    };
  }
  ...
  return {
    filePath,
    changed: false,
    reason: 'No Session Update mock infrastructure issues found or getSessionWorkdir already exists'
  };
}

export function fixSessionUpdateMockInfrastructureTests(filePaths: string[]): SessionUpdateMockFixResult[] {
