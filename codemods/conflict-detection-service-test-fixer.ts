...
export function fixConflictDetectionTestExpectations(
  sourceFile: SourceFile
): ConflictDetectionTestFixResult {
          ...
          if (memberCall && memberCall.getKind() === SyntaxKind.CallExpression) {
            ...
            if (methodName === "toBe") {
              ...
              if (toBeLiterals.length > 0) {
                ...
                if (
                  expectArg.includes("conflictType") &&
                  currentValue === "ConflictType.ALREADY_MERGED"
                ) {
                ...
                if (
                  (expectArg.includes("shouldMerge") ||
                    expectArg.includes("canProceed") ||
                    expectArg.includes("success")) &&
                  currentValue === "false"
                ) {
                  newValue = "true";
                }
...
export function fixConflictDetectionServiceTests(
  testFiles: string[]
): ConflictDetectionTestFixResult[] {
