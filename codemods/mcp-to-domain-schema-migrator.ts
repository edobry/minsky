...
import { Project, SourceFile, ImportDeclaration, CallExpression, SyntaxKind, Node } from "ts-morph";
...
export class McpToDomainSchemaMigrator {
      ...
      try {
        ...
        try {
          ...
          if (result) {
            ...
            const fileChanges =
              result.importChanges + result.schemaChanges + result.responseBuilderChanges;
  ...
  private isMcpFile(sourceFile: SourceFile): boolean {
    ...
    return imports.some((imp) => {
      ...
      return (
        moduleSpecifier.includes("./schemas/common-parameters") ||
