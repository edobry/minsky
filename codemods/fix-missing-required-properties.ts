  ...
  try {
    ...
    if (fileName === "init.ts") {
      ...
      for (const objLiteral of objectLiterals) {
        ...
        const hasTransport = properties.some(
          (prop) =>
            prop.getKind() === SyntaxKind.PropertyAssignment &&
        ...
        if (hasEnabled && !hasTransport) {
          ...
          const enabledProp = properties.find(
            (prop) =>
              prop.getKind() === SyntaxKind.PropertyAssignment &&
              prop.asKindOrThrow(SyntaxKind.PropertyAssignment).getName() === "enabled"
          );
    ...
    if (fileName === "conflict-detection.ts") {
      ...
      for (const returnStmt of returnStatements) {
        ...
        if (expression && expression.getKind() === SyntaxKind.ObjectLiteralExpression) {
          ...
          const hasEnhancedMergeProps = properties.some(
            (prop) =>
              prop.getKind() === SyntaxKind.PropertyAssignment &&
              ["merged", "conflicts"].includes(
                prop.asKindOrThrow(SyntaxKind.PropertyAssignment).getName()
              )
          );

          const hasSmartUpdateProps = properties.some(
            (prop) =>
              prop.getKind() === SyntaxKind.PropertyAssignment &&
              ["updated", "skipped"].includes(
                prop.asKindOrThrow(SyntaxKind.PropertyAssignment).getName()
              )
          );

          const hasWorkdir = properties.some(
            (prop) =>
              prop.getKind() === SyntaxKind.PropertyAssignment &&
    ...
    if (fileName === "localGitBackend.ts" || fileName === "remoteGitBackend.ts") {
      ...
      for (const returnStmt of returnStatements) {
        ...
        if (expression && expression.getKind() === SyntaxKind.ObjectLiteralExpression) {
          ...
          const hasBranch = properties.some(
            (prop) =>
              prop.getKind() === SyntaxKind.PropertyAssignment &&
