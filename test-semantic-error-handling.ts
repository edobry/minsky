...
async function testFilesystemErrorClassification() {
  ...
  console.log(`   Solutions Include createDirs: ${dirResult.solutions.some(s => s.includes("createDirs"))}`);
...
async function testSessionErrorClassification() {
  ...
  console.log(`   Solutions Include session_list: ${sessionResult.solutions.some(s => s.includes("session_list"))}`);
...
async function testContextAwareSolutions() {
  ...
  console.log(`   Contains createDirs suggestion: ${result.solutions[0].includes("createDirs: true")}`);
