...
async function testSessionCommands() {
  try {
    ...
    if (pwd.includes("/minsky/git/") && pwd.includes("/sessions/")) {
      ...
      console.error(
        "❌ Not in a session workspace. Please run this script from within a session directory."
      );
      console.error(
        "   For example: cd $(minsky session dir task#XXX) && bun run test-session-detection.ts"
      );
      process.exit(1);
    }
