...
async function testSessionCommands() {
  ...
  const dirIgnoreResult = spawnSync(
    "bun",
    ["run", "./src/cli.ts", "session", "dir", "--ignore-workspace"],
    {
      encoding: "utf-8",
      stdio: "pipe",
    }
  ...
  console.log(
    `Status: ${dirIgnoreResult.status !== 0 ? "✅ Expected failure" : "❌ Unexpected success"}`
  );
  ...
  const getIgnoreResult = spawnSync(
    "bun",
    ["run", "./src/cli.ts", "session", "get", "--ignore-workspace"],
    {
      encoding: "utf-8",
      stdio: "pipe",
    }
  ...
  console.log(
    `Status: ${getIgnoreResult.status !== 0 ? "✅ Expected failure" : "❌ Unexpected success"}`
  );
