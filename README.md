...
Minsky provides AI-powered completion and analysis capabilities through multiple provider backends.

#### `minsky ai complete [options]`

Generate text completions using various AI providers (OpenAI, Anthropic, Google).

```bash
# Basic completion
minsky ai complete --prompt "Explain how Git works"

# Specify provider and model
minsky ai complete --prompt "Review this code" --provider openai --model gpt-4o

# Use custom system prompt
minsky ai complete --prompt "Debug this error" --system "You are a senior developer"

# Stream response
minsky ai complete --prompt "Write a function" --stream

# Set parameters
minsky ai complete --prompt "Generate docs" --temperature 0.7 --max-tokens 1000
```

Options:
- `--prompt <text>`: Text prompt for completion (required)
- `--provider <name>`: AI provider (openai, anthropic, google)
- `--model <name>`: Specific model to use
- `--system <text>`: System prompt for context
- `--temperature <num>`: Randomness (0.0-2.0)
- `--max-tokens <num>`: Maximum tokens to generate
- `--stream`: Stream response in real-time

#### `minsky ai models [options]`
...
# Filter by provider
minsky ai models --provider openai

# JSON output
minsky ai models --format json
```
...
#### `minsky ai validate [options]`

Validate AI configuration and test provider connections.
...
minsky ai validate --provider anthropic
```
...
AI providers are configured through Minsky's standard configuration system:
...
- `gemini-1.5-flash-latest` - Fast Gemini variant
