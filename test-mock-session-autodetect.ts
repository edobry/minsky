...
const explicitResult = spawnSync("bun", ["run", TEST_HELPER_PATH, "session", "get", "explicit-session"], {
  encoding: "utf-8",
  env: { ...process.env, ...ENV_VARS }
});
...
const ignoreResult = spawnSync("bun", ["run", TEST_HELPER_PATH, "session", "get", "--ignore-workspace"], {
  encoding: "utf-8",
  env: { ...process.env, ...ENV_VARS }
});
