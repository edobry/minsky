...
class DependencyBackfillTool {
  ...
  private async processTask(task: any): Promise<void> {
    try {
      ...
      const invalidReferences: TaskDependencyReference[] = [];
      for (const ref of references) {
        ...
        if (exists) {
          ...
          // Track invalid references for cleanup
          invalidReferences.push(ref);
          console.warn(`⚠️  Task ${task.id} references non-existent task ${ref.referencedTaskId} - will be removed from spec`);
        }
      }
      ...
      // Clean up the spec by removing redundant dependency text AND invalid references
      await this.cleanupTaskSpec(task.id, specContent, validReferences, invalidReferences);
    } catch (error) {
      this.results.errors.push({
        taskId: task.id,
  ...
  /**
   * Clean up task spec by removing redundant dependency references AND invalid references
   */
  private async cleanupTaskSpec(
    taskId: string,
    originalContent: string,
    validReferences: TaskDependencyReference[],
    invalidReferences: TaskDependencyReference[] = []
  ): Promise<void> {
    ...
    // Remove invalid task references (sort by position descending to avoid index issues)
    const allInvalidRefs = [...invalidReferences].sort((a, b) => b.startIndex - a.startIndex);
    for (const ref of allInvalidRefs) {
      // Remove the specific invalid reference text
      const beforeRef = cleanedContent.slice(0, ref.startIndex);
      const afterRef = cleanedContent.slice(ref.endIndex);
      cleanedContent = beforeRef + afterRef;
      console.log(`    🗑️  Removed invalid reference to ${ref.referencedTaskId} from ${taskId}`);
    }
