...
async function testSessionEditWithRealKey() {
  ...
  try {
    ...
    if (config.ai?.providers?.morph) {
      console.log("📋 **Current Morph API Key:**", 
        (config.ai.providers.morph as any)?.apiKey ? 
        `${(config.ai.providers.morph as any).apiKey.substring(0, 12)}...` : 'missing');
      
      // Override with real key (from global config we saw earlier)
      (config.ai.providers.morph as any).apiKey = "sk-S-E_EIrfm3MSanIWZDOVDOo-O8ABFsrzlgo1MtaXmLzYBRNz";
