...
- **Configuration Directory**: Improved configuration directory handling to use proper XDG standards (`XDG_CONFIG_HOME` or `~/.config`) instead of hardcoding paths. Added protection against user `NODE_CONFIG_DIR` override attempts with appropriate warnings to prevent configuration system interference.
