 ...
 */
async function migrateTasklessSessions(
  options: {
    dryRun?: boolean;
    autoDelete?: boolean;
    verbose?: boolean;
...
async function displayMigrationReport(report: MigrationReport): Promise<void> {
  ...
  if (report.safeToDelete.length > 0) {
    ...
    for (const session of report.safeToDelete) {
      log.cli(
        `  • ${session.record.session} (${formatBytes(session.directorySize)}, last modified: ${session.lastModified.toLocaleDateString()})`
      );
    }
  }
  ...
  if (report.requiresManualReview.length > 0) {
    ...
    for (const session of report.requiresManualReview) {
      ...
      log.cli(
        `  • ${session.record.session} (${formatBytes(session.directorySize)}) - ${reasons.join(", ")}`
      );
  ...
  if (report.summary.canAutoDelete > 0) {
    log.cli(
      `Run with --auto-delete --no-dry-run to clean up ${report.summary.canAutoDelete} empty sessions`
    );
  }
