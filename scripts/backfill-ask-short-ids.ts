#!/usr/bin/env bun
/**
 * One-time backfill: assign a numeric `ask#N` short id (mt#2965, ADR-029) to
 * every existing ask row created before the `short_id` column shipped.
 *
 * The schema migration (0065) is additive-nullable — it does NOT backfill.
 * `DrizzleAskRepository.create` mints `ask#N` only for NEW rows going
 * forward (mt#2965). This script is the separate, explicitly-run operational
 ...
 *
 * Assignment order: every ask WITHOUT a short_id, sorted by `createdAt`
 * ascending (oldest first), numbered sequentially starting after the
 * highest already-assigned `ask#N` (so ids minted by `create()` between
 ...
 *
 * Usage:
 *   bun scripts/backfill-ask-short-ids.ts              # dry-run (default)
 *   bun scripts/backfill-ask-short-ids.ts --execute     # apply
 *
 ...
 *
 * @see mt#2965 — this script's originating task
 * @see mt#2963 — short-id.ts (nextShortId), short-id-column.ts (schema pattern)
 * @see packages/domain/src/storage/migrations/pg/0065_kind_queen_noir.sql — the additive column+index migration
 */
...
// portion of the mt#2965 test suite).
// ---------------------------------------------------------------------------

/** Minimal shape this script needs from an ask row to plan assignments. */
...
/** One planned `id -> ask#N` assignment. */
...
/**
 * Plan `ask#N` assignments for every row lacking a short_id, oldest-first by
...
export function planBackfillAssignments(rows: BackfillCandidateRow[]): BackfillPlan {
  ...
  // create() (e.g. a new ask created after migration-deploy but before this
  // script ran). Grows as each assignment is planned so ids stay monotonic
  // and unique WITHIN the batch too.
  ...
  for (const row of sorted) {
    const next = nextShortId("ask", liveIds, []);
...
async function bootstrapDb(): Promise<PostgresJsDatabase> {
  ...
  // Duck-typed guard (PR #2110 R1), not `instanceof PersistenceProvider`: an
  // `instanceof` check against a class pulled in via a dynamic `import()` can
  // false-negative when the resolved object was constructed from a
  // DIFFERENT instance of the same module (dual-package hazard) — the check
  // then silently rejects a perfectly valid provider. Check for the actual
  // capability/method this script needs instead, matching the established
...
async function fetchAllAskRows(db: PostgresJsDatabase): Promise<BackfillCandidateRow[]> {
  const rows = await db
    .select({
      id: asksTable.id,
      shortId: asksTable.shortId,
      createdAt: asksTable.createdAt,
    })
    .from(asksTable);
...
async function main(): Promise<void> {
  ...
  const rows = await fetchAllAskRows(db);
  const plan = planBackfillAssignments(rows);

  console.log(`backfill-ask-short-ids ${execute ? "(EXECUTE)" : "(dry-run)"}`);
  console.log(`  total asks:              ${plan.total}`);
  ...
  if (execute) {
    for (const a of plan.assignments) {
      try {
        const updated = await db
          ...
          .where(and(eq(asksTable.id, a.id), isNull(asksTable.shortId)))
    ...
    // Verification (per mt#2965 acceptance: "count matches"): recount rows
    // still missing a short_id after the run.
    const postRows = await fetchAllAskRows(db);
