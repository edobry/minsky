...
async function main(): Promise<void> {
  ...
  // 9. Summarize gaps that are now known without needing to try them
  console.log("\nStep 9: Known gaps (from mt#054 audit; would need to be tested to confirm)");
  observe(
    "known-gap",
    "gap",
  ...
  // 10. Cleanup — mark task as CLOSED so it doesn't clutter the task list
  console.log("\nStep 10: Close the PoC task");
  try {
    await client.callTool({
      name: "tasks.status.set",
