/**
 * Unit tests for `backfill-memory-short-ids.ts`'s pure planning logic (mt#2966).
 *
 * `planBackfillAssignments` is exercised directly — no DB, no I/O — covering:
 *   - sequential `mem#N` assignment ordered by createdAt ascending
...
import { planBackfillAssignments, type BackfillCandidateRow } from "./backfill-memory-short-ids";

function row(id: string, createdAt: string, shortId: string | null = null): BackfillCandidateRow {
  return { id, shortId, createdAt };
}

describe("planBackfillAssignments (mt#2966)", () => {
  it("assigns mem#1, mem#2, ... in createdAt-ascending order", () => {
    const rows = [
    ...
    expect(plan.assignments).toEqual([
      { id: "uuid-a", shortId: "mem#1" },
      { id: "uuid-b", shortId: "mem#2" },
      { id: "uuid-c", shortId: "mem#3" },
    ]);
  });

  it("is idempotent: rows that already have a short_id are skipped, not reassigned", () => {
    const rows = [
      row("uuid-a", "2026-01-01T00:00:00.000Z", "mem#1"),
    ...
    expect(plan.assignments).toEqual([{ id: "uuid-b", shortId: "mem#2" }]);
  });

  it("continues the sequence after the highest already-assigned mem#N, not from 1", () => {
    // Simulates: memories created via mint-on-create (mt#2966) after migration
    // deploy but before this backfill ran — the backfill must not reissue
    // mem#1..mem#5.
    const rows = [
      row("uuid-old", "2026-01-01T00:00:00.000Z", null),
      row("uuid-new-1", "2026-02-01T00:00:00.000Z", "mem#5"),
      row("uuid-new-2", "2026-02-02T00:00:00.000Z", "mem#6"),
    ];

    const plan = planBackfillAssignments(rows);

    expect(plan.alreadyAssigned).toBe(2);
    expect(plan.assignments).toEqual([{ id: "uuid-old", shortId: "mem#7" }]);
  });

  it("returns an empty plan when every row already has a short_id", () => {
    const rows = [
      row("uuid-a", "2026-01-01T00:00:00.000Z", "mem#1"),
      row("uuid-b", "2026-01-02T00:00:00.000Z", "mem#2"),
    ];
  ...
  it("produces distinct, monotonically increasing short ids within one batch", () => {
    ...
    expect(shortIds).toEqual(Array.from({ length: 10 }, (_, i) => `mem#${i + 1}`));
  });
});
