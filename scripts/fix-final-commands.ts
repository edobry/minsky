...
function fixCommandParams(content: string): { content: string; changeCount: number } {
  ...
  const patterns = [
    ...
    {
      pattern:
        /execInRepository: createMock\(\(_workdir, _command\) => \{([^}]*?)command([^}]*?)\}/gs,
      replacement: "execInRepository: createMock((_workdir, command) => {$1command$2}",
    },
    // mockExecAsync with _command parameter
    {
      pattern:
        /mockExecAsync = createMock\(async \(_command: unknown\) => \{([^}]*?)command([^}]*?)\}/gs,
      replacement: "mockExecAsync = createMock(async (command: unknown) => {$1command$2}",
    },
    // conflictMockExecAsync pattern
    {
      pattern:
        /const conflictMockExecAsync = createMock\(async \(_command: unknown\) => \{([^}]*?)command([^}]*?)\}/gs,
      replacement:
        "const conflictMockExecAsync = createMock(async (command: unknown) => {$1command$2}",
    },
  ];
