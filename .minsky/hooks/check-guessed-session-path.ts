...
//
// ADR-028 Phase 1 pilot guard (mt#2650): this file also exports `run()`, the
// dispatcher-compatible pure-function entry point registered in
// `./registry.ts` and invoked in-process by `./dispatch-pretooluse.ts`. The
// standalone `if (import.meta.main)` block below is UNCHANGED and still
// works for direct CLI invocation — both paths share the same pure logic
// (`findMissingInToolInput` / `buildDenialReason`), matching the ADR's
// documented Phase 1-5 posture that guards keep their independent
// CLI-invocability until Phase 6.
//
...
// ---------------------------------------------------------------------------
// Dispatcher-compatible pure function (ADR-028 D1/D2 — mt#2650)
// ---------------------------------------------------------------------------

/**
 * Guard-dispatcher entry point. Pure relative to its inputs — the only I/O
 * is `existsSync` inside `findMissingInToolInput` (unchanged from the
 * standalone path) and reading `process.env[OVERRIDE_ENV_VAR]` (the guard's
 * OWN legacy override, honored independently of the dispatcher's unified
 * `MINSKY_HOOK_OVERRIDE` check — per ADR-028's "legacy per-guard env vars
 * remain honored by the guards themselves" until the Phase 7 deprecation
 * shim). Returns `null` for "no output" (silent allow), matching the
 * dispatcher's `GuardRunResult` contract.
 */
export function run(input: ToolHookInput, _ctx: DispatchContext): GuardOutcome | null {
  ...
  if (missing.length > 0) {
    return { deny: { reason: buildDenialReason(missing) } };
  }
  return null;
}
