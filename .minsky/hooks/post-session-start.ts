...
async function main(): Promise<void> {
  ...
  // Sanitize before composing the /tmp label-file path — raw session_id with
  // `/` or `..` would allow path traversal (mt#2304 R3; same allowlist as
  // two-strikes-record). Must match auto-session-title.ts's sanitization so the
  // write/read filenames agree.
