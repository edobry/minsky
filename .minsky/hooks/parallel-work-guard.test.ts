...
describe("extractScopeConstraintsFiles / session_generate_prompt contract (mt#2811)", () => {
  it("parses the exact '## Scope Constraints' shape session_generate_prompt renders", () => {
    const scopeFiles = [
      "/Users/test/.local/state/minsky/sessions/fake-session/src/foo.ts",
      "/Users/test/.local/state/minsky/sessions/fake-session/src/bar.ts",
    ];

    const result = generateSubagentPrompt({
      sessionDir: "/Users/test/.local/state/minsky/sessions/fake-session",
    ...
    // the contract covers the actual production entrypoint.
    const { files, warnings } = extractInScopeFiles(result.prompt);
    expect(warnings).toHaveLength(0);
...
`;
  ...
  it("falls back to '## Context' backtick-path scan when '**In scope:**' is prose-only", () => {
    const { files, warnings } = extractInScopeFiles(mt2811RealSpec);
    expect(files).toContain(".claude/hooks/parallel-work-guard.ts");
    ...
    expect(warnings.some((w) => /Could not extract file paths/.test(w))).toBe(true);
    expect(warnings.some((w) => /Context.*backtick-path scan/.test(w))).toBe(true);
  });
...
`;
    ...
    expect(warnings.some((w) => /no extractable file references/i.test(w))).toBe(true);
  });
...
`;
    const { files, warnings } = extractInScopeFiles(spec);
    expect(files).toEqual([]);
    const terminal = warnings[warnings.length - 1] ?? "";
...
`;
    const { files, warnings } = extractInScopeFiles(spec);
    expect(warnings).toHaveLength(0);
    expect(files).toEqual(["src/domain/foo.ts", "src/domain/bar.ts"]);
  });
});
