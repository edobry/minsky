 ...
 * render at 1327 — over.
 *
 * So {@link truncateToCodePoints} currently has NO call sites. It is kept rather
 * than deleted because it remains the correct primitive for a value that is NOT
 * interpolated into ceiling-enforced text, and because the contrast between the
 * two is what makes the unit choice legible at a future call site. If you are
 * adding one, the question to answer first is whether anything measures the
 * result with `.length` — if so, you want THIS function.
 */
