...
name: retrospective
description: >-
  Structured post-failure analysis producing durable fixes to process artifacts.
  Auto-triggers on user correction signals and on agent self-recognized failure signals —
  match by meaning, not literal string. Use when a post-merge audit finds issues, the same
  feedback recurs, a subagent returns incomplete work, or the user asks what went wrong.
user-invocable: true
---
