...
**Does this PR MODIFY an existing test file, and is it a bugfix?** (mt#3244) If the title carries a `fix(` type — or the bound task's spec describes a defect — the evidence block should also record a **negative control**: the changed test run against the un-fixed tree, observed FAILING. A test written after the fix passes whether or not the fix works, so a passing run alone says nothing about it. **Where the label goes (mt#3511, corrected mt#3584):** on its own line, NOT inside a code fence. That is the only placement rule — the label is matched anywhere in the PR body, so it need not sit inside the evidence block (beside the run output just reads best). A fenced marker reads as quoted text and will not match. Accepted forms: `Negative control:`, `Negative control — <subject>` (em or en dash, for a PR with several), `Failing-first:` in either form, or a heading naming either; a leading `-` bullet and `**bold**` are fine, and the run output beneath MAY be fenced. Use `[negative-control-deferred: mt#NNNN]` if it truly cannot run pre-merge. Log-only today — it warns, it does not block.
