...
**The signal to wait for is \`pushed: true\`, not the local commit hash.** A backgrounded \`session_commit\` completes later with its own notification; until that notification reports \`pushed: true\` (or \`pushConfirmedVia\`), the remote may not have your commit. If a wait times out with \`expectedHeadShaUnreached\` in the payload, the push is the problem, not the reviewer — go check the backgrounded commit rather than waiting longer or bypassing.
