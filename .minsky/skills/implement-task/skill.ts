...
7. **Added or modified a test file? (mt#2530)** Two already-shipped gates fire at MERGE on any PR that adds or changes a test file. Front-load BOTH in the PR body BEFORE \`session_pr_create\`, or they surface reactively as sequential merge blocks — each costing a full round:

   - **(a) No placeholder assertions** in the new/changed tests. The Prevent-Placeholder-Tests CI check (mt#1938) blocks on \`expect(true).toBe(true)\`, \`.skip\`, \`.todo\`, and empty test bodies. For a compile-time-only test (e.g. a \`// @ts-expect-error\` type-guard assertion), PAIR the directive with a REAL runtime assertion so the test still exercises executable behavior.
   - **(b) An \`Execution evidence:\` block in the PR body** — the LITERAL heading WITH the colon. The mt#1459 merge gate matches the exact string \`Execution evidence:\`, NOT \`## Execution evidence\` (a markdown heading without the trailing colon does NOT satisfy it). Under it, paste the ACTUAL test-run output for the new/changed test files (\`bun test … <files>\` — pass/fail counts + file names), not a promise to run them.

   Origin: mt#2524 (2026-06-19) hit three sequential reactive merge blocks in one session — placeholder assertions, then execution-evidence-missing, then a heading without the colon — each fixed in its own round. mt#2525 (2026-06-21) recurred on the missing \`Execution evidence:\` block at merge. Front-loading removes the reactive rounds. Gates: mt#1459 (\`Execution evidence:\` merge gate), mt#1460 (/prepare-pr sibling), mt#1938 (Prevent-Placeholder-Tests CI check).
