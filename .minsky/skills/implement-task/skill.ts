...
7. **Added a new test file? (mt#2530)** Two already-shipped gates fire at MERGE when a PR adds a NEW test file. Front-load BOTH in the PR body BEFORE \`session_pr_create\`, or they surface reactively as sequential merge blocks — each costing a full round:
   ...
   - **(e) Negative control on a bugfix (mt#3244, calibration-first).** If the PR is **bugfix-shaped** — a \`fix(\` conventional-commit type on the title, or a bound task whose spec describes a defect — and it **modifies an existing test file**, the \`Execution evidence:\` block should also record a **negative control**: the changed test run against the UN-FIXED tree, observed FAILING. Revert the fix (or stub the condition), run the test, paste the failure alongside the passing run. **Where the label goes (mt#3511):** on its OWN line, NOT inside a code fence — put it just above the fenced run output, not within it. A fenced marker is treated as quoted text and will not match, which is the trap the old wording ("inside the evidence block") set. Accepted forms (case-insensitive): \`Negative control: <what you reverted and what failed>\`, \`Negative control — <subject>\` (em or en dash — use this when the PR has several, each with its own subject), \`Failing-first:\` in either form, or a Markdown heading naming either. A leading \`-\` bullet and surrounding \`**bold**\` are fine. The failing run itself MAY be fenced beneath the label. If it genuinely cannot be run pre-merge, use \`[negative-control-deferred: mt#NNNN]\` naming a tracking task — prose does not count. Log-only per the mt#2263 ladder. Override: \`MINSKY_SKIP_TEST_FIRST_EVIDENCE=1\`.

     **What a negative control does NOT buy you (mt#3574).** It proves the probe CAN fail. It does not prove the probe is observing the RIGHT SYSTEM — a control run in a substitute runtime is just as blind to a substrate mismatch as the passing run beside it. See item 8's **Substrate direction**.
