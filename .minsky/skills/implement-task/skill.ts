...
4. **Guard/rule documentation (mt#2208).** If this PR adds or modifies a pre-commit guard, a Claude Code hook (\`.claude/hooks/*\`), an ESLint rule, or any mechanism whose siblings are documented in a \`.minsky/rules/*.mdc\` section (e.g., the pre-commit guards documented in \`hook-files.mdc\`), document the new mechanism in the SOURCE rule and recompile IN THIS PR — before \`session_pr_create\`. Skipping this means the reviewer-bot's \`## Documentation impact\` check returns BLOCKING at review time, costing a full extra round.
   ...
   - Edit the canonical source — \`.minsky/rules/<file>.mdc\` — NOT the generated \`CLAUDE.md\` / \`AGENTS.md\` / \`.cursor/rules/\` outputs. Then compile. **From a session, use the MCP tool with routing: \`mcp__minsky__compile({ task: "mt#<id>" })\`** (mt#4394 — the tool takes \`task\` / \`sessionId\` / \`workspace\` on the same contract as \`validate_typecheck\`, and returns \`compiledWorkspace\`; read it and confirm it is the session directory, not main). Before mt#4394 the MCP tool compiled only the server's cwd, so this step said to run \`bun run src/cli.ts compile\` via \`session_exec\` — that shell-out is now a genuine CLI-for-MCP substitution and the \`cli-mcp-substitution\` detector is right to flag it; the CLI form remains the way to compile the MAIN workspace from a terminal. A bare invocation regenerates every target that already has output on disk — \`claude.md\`, \`agents.md\`, \`claude-rules\`, \`cursor-rules-ts\`, and the skill/agent/hook targets — in one pass, and reports each target it wrote (mt#2803).
