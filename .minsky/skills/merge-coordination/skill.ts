...
This is the in-band replacement for the raw \`gh api PUT\` below — no hand-run CLI, and Minsky session cleanup still fires. It requires a non-empty \`bypassReason\` and a present (non-DISMISSED) \`CHANGES_REQUESTED\` review (the false-positive / self-reversal / leakage-stale case; for a review that EXISTS but is stale against HEAD, \`acceptStaleReviewerSilence\`), refuses on failing status checks or other merge blockers, auto-dismisses the blocking review using \`bypassReason\` as evidence, writes the canonical audit signature into the merge-commit body, and always uses \`merge_method=merge\`. Use it whenever the bypass conditions below are met and the agent has merge rights.
