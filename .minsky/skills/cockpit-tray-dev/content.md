...
1. **The cockpit WEB app** (`src/cockpit/web/**`, `src/cockpit/server.ts`) — served by the daemon. A running tray app **auto-rebuilds the web bundle** (mt#2297) and **auto-restarts the daemon on backend changes** (mt#2299) when source changes (e.g. after `git pull`) — any non-test module under `src/**` or `packages/*/src`, not just `src/cockpit` (mt#4230, mt#5060). So a cockpit-web change needs **no manual rebuild** — refresh the window, or let the watcher fire; the "Last build" / "Daemon uptime" menu lines confirm currency.
