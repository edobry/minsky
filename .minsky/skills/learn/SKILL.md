...
| **Rule**    | Rule-admission ladder rung 1 or 5 (mt#2874): file-shaped (`globs`) or genuinely per-turn (`alwaysApply: true`, mt#1876 criterion)                                                                                                                        | Session-backed: edit the `.minsky/rules/<name>.mdc` source, `compile` (NOT the retired `rules compile`, which regenerates nothing — mt#3309; then verify EVERY target output regenerated, not just `CLAUDE.md` — see `implement-task §7` step 4), commit both — via a filed edit task, or directly if already in a suitable session                                                                                                                                               | Same as skill: search for an existing open edit task on the same rule file first                                                                                             |
