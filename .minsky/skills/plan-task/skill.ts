...
\`## Planning finding\`) and point at it with a \`minsky://task/<id>\` deeplink, per
\`communication-contract.mdc\` ("detail lives behind a pointer, never inline"). Only detail short
enough to belong in chat on its own renders there, as an ordinary markdown section beneath the
lead. The same prohibition covers every other HTML affordance in chat (\`<br>\`, \`<sub>\`,
inline \`<table>\`); PR bodies and Notion pages are a different surface, where \`<details>\` does
