...
3. **Wait for the terminal content, THEN snapshot and look.** Call `take_snapshot` (a11y-tree view; preferred over screenshots for textual reasoning) or `take_screenshot` (visual, when you need pixel-level review).

   **Wait on the CONTENT SELECTOR, never on a fixed sleep.** A cockpit surface that fetches data renders a skeleton first. Sleeping a couple of seconds and then capturing catches whatever state the page happened to be in, and a transition state photographs exactly as confidently as the real thing. Use `wait_for` against the terminal content — the rendered turn, the populated row, the actual card value — or poll it with `evaluate_script`. `Loading conversation…` is not evidence, and neither is the tab chrome having appeared.

   Origin: mt#2398 / PR #1653. The pre-merge "live verification" screenshotted the `Loading conversation…` state and the two-tab chrome, and the route 404'd on essentially every real click once merged (mt#2420).
...
### Verifying a render change, and handing it over (mt#2421)

Steps 1–4 get you looking at the cockpit. This is what to do when the thing you changed is what the principal will look at. It applies to any user-facing render change, whether or not the PR carries tests — tests are not what makes this unnecessary, they are what makes it feel unnecessary (mt#3810's component tests ran in happy-dom, which never decodes or lays out an image).

1. **Exercise the surface the way a user does.** Click the row in the list; do not hand-type a known-good id or URL. Hand-typing proves the renderer works and says nothing about the wiring that gets a user there — which is precisely the half that broke in mt#2398, where two disjoint id-spaces meant every real click 404'd while the renderer itself was fine.
2. **Assert the end state, not a proxy.** Read back the actual thing: the image's `complete` flag and decoded dimensions, the turn's text, the row's values. "It builds", "the route loads", "the tab opened" are all compatible with the surface being broken.
3. **Capture the FULL render at a realistic viewport (≥1440×900), uncropped.** A favorable crop is how a sparse or broken layout passes review — a cropped 1440×900 capture once hid a vast dead space that the principal saw immediately in a full-window shot.
4. **Present it without an aesthetic verdict.** Report what you observed and show the render; do not write "reads as composed", "clears the jank bar", or "looks good". Whether it LOOKS right is the principal's call, not yours to certify — see the humility stakes-filter (mt#2386). Objective defects are yours to catch; subjective quality is theirs to judge.
5. **Hand over a link.** Put a URL the principal can open — deployed or localhost, pointing at the changed surface — in the PR body and in your report. For a defect they reported visually, the cheapest possible evidence is the thing itself; mt#3810 produced test output, a bundle grep and an API response, and none of them let the principal see their fix. The mt#2421 calibration surface in `require-execution-evidence-before-merge.ts` WARNs when a render-path PR body carries no URL and no image.
