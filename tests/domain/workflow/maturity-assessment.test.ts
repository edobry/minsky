...
describe("MATURITY_CATEGORIES", () => {
  it("has correct categories with proper weights", () => {
    expect(MATURITY_CATEGORIES["Code Quality"]).toBe(0.20);
    expect(MATURITY_CATEGORIES["Testing"]).toBe(0.20);
    expect(MATURITY_CATEGORIES["Dependency Management"]).toBe(0.15);
    expect(MATURITY_CATEGORIES["Security"]).toBe(0.15);
    expect(MATURITY_CATEGORIES["Task Management"]).toBe(0.10);
    expect(MATURITY_CATEGORIES["Development Workflow"]).toBe(0.10);
    expect(MATURITY_CATEGORIES["Documentation"]).toBe(0.10);
  });
...
describe("performMaturityAssessment", () => {
  const createMockWorkflow = (name: string, tool: string, categories: string[] = []): ParsedWorkflowConfig => ({
    name,
  ...
  it("generates relevant recommendations", () => {
    ...
    const assessment: MaturityAssessment = {
      ...
      recommendations: [
        ...
        { category: "Testing", action: "Configure unit tests", command: "minsky workflow add test" },
        { category: "Security", action: "Add secret scanning", command: "minsky workflow add security" }
      ]
    };
